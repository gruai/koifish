...
struct NP_ {  // Paramer of GeNeuron
...
struct SAM_encoder : public Fish {
    ...
    SAM_encoder(void* ctx_, bool grad_, int nEmbed, int nLayer, int n_enc_head_dim, int _enc_out_chans, int n_pt_embd, int _img_embd, int _window_size,
                int _patch_size, int flag = 0x0)
        : /*Fish("SAM_encoder",ctx_,grad_),*/ nEmbed(nEmbed),
          head_dim(n_enc_head_dim),
          n_window_size(_window_size),
          n_img_embd(_img_embd),
          n_patch_size(_patch_size),
...
inline hGensor sam_layer_norm_2d(void* ctx0, hGensor layer, int n_channels, hGensor w, hGensor b, float eps) {
    ...
    layer = ggml_permute(ctx0, ggml_norm(ctx0, ggml_cont(ctx0, ggml_permute(ctx0, layer, 1, 2, 0, 3)), eps), 2, 0, 1, 3);

    layer = ggml_add(ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, ggml_reshape_3d(ctx0, w, 1, 1, n_channels), layer), layer),
...
struct SegmentAnything : public Fish {
    ...
    bool Load(const std::string& mode_path, const ggml_ftype qtype, int flag = 0x0) {
        ...
        switch (n_enc_state) {
            ...
            case 1024:
                global_attn_indices = {5, 11, 17, 23};
            case 1280:
        ...
        enc = std::make_shared<SAM_encoder>(ctx, false, n_enc_state, n_enc_layer, n_enc_head_dim, n_enc_out_chans, n_pt_embd, n_img_embd, n_window_size,
                                            n_patch_size);
        ...
        for (int i = 0; i < n_enc_layer; ++i) {
            ...
            NT_SAM* hFormer =
                new NT_SAM(enc, "image_encoder.blocks." + std::to_string(i), {n_enc_state, n_enc_head_dim, n_img_embd, n_window_size}, is_global_attn, 0x0);
            enc->AddLayer(std::shared_ptr<NeLayer>(hFormer));
        }
        // prompt encoder
        enc_prompt = nullptr; /*std::make_shared<Fish>("enc_prompt",ctx,0x0);*/
        childs.push_back(enc_prompt);
        ...
        // mask decoder
        nnDec = std::make_shared<Fish>("nnDec", ctx, 0x0);
        childs.push_back(nnDec);
    ...
    void EncodeImage(int nx, int ny, const std::vector<float>& data, int flag = 0x0) {  // Build graph of encoder of image
        ...
        struct ggml_init_params ggml_params = {
            state.buf_compute_img_enc.size(),
            state.buf_compute_img_enc.data(),
            true,
        };
        ...
        T873:leaf_178: [ 64  64  256  1 f32] sum=12757.2 data=[-0.994997-0.963888] rZ=0%
            ...
            //openblas
             {-0.05101 -0.06349 -0.07116 -0.06837 -0.06826 -0.06974 -0.07149 -0.07087 -0.06775 -0.05427 -0.06469 -0.06627 -0.06987 -0.06701 -0.07403 -0.07661
        -0.07799 -0.07747 -0.07566 -0.07484 -0.07647 -0.07696 -0.07568 -0.07464 -0.07195 -0.06934 -0.06083 -0.05930 -0.07135 -0.07290 -0.07332 -0.07152 ...
                    0.01642 0.01482 0.01381 0.01452 0.01562 0.01324 0.01643 0.01636 0.01630 0.01915 0.01828 0.01951 0.02027 0.01687 0.01412 0.01521 0.01351
