...
template <typename T>
constexpr int TILE_COL_DIM = sizeof(T) == 1 ? 32 : 16;
template <typename T>
...
struct DEFAULT_ALIGN cuTILE : public TILE<_T, _rows, _cols> {
    ...
    static constexpr int swizzle_bytes = (sizeof(dtype) == 1   ? (  // Add FP8 case
                                          ...
                                          : sizeof(dtype) == 2 ? (underlying_width % 4 == 0   ? 128
                                                                  : underlying_width % 2 == 0 ? 64
                                                                                              : 32)
                                          : sizeof(dtype) == 4 ? (underlying_width % 2 == 0 ? 128 : 64)
                                                               : -1);
    ...
    __device__ inline dtype& operator[](const int2& rowcol) { return *idx(data, rowcol); }
    __device__ inline const dtype& operator[](const int2& rowcol) const { return *(const dtype*)idx((dtype*)data, rowcol); }
    __device__ inline dtype& operator[](int idx) { return data[idx]; }
    __device__ inline const dtype& operator[](int idx) const { return data[idx]; }
    ...
    using subtile = st_subtile<cuTILE<T, rows, cols>, subtile_rows, subtile_cols>;  ///< A templated subtile type wrapper for this tile.
};

template <int _height, int _width>
using st_bf = st<bf16, _height, _width>;
template <int _height, int _width>
using st_hf = st<half, _height, _width>;
template <int _height, int _width>
using st_fl = st<float, _height, _width>;
template <int _height, int _width>
using st_fl8_e4m3 = st<fp8e4m3, _height, _width>;
template <int _height, int _width>
using st_fl8_e5m2 = st<fp8e5m2, _height, _width>;