...
static uint32_t n_ctx  = 512;
static uint32_t n_head = 32;
static uint32_t n_mult = 2;
...
static struct ggml_tensor *randomize_tensor(struct ggml_tensor *tensor, int ndims, const int64_t ne[], float fmin, float fmax) {
    switch (ndims) {
        case 1:
...
static uint32_t get_n_ff(const struct llama_hparams *hparams) {
    int n_mult          = 4;
    const uint32_t n_ff = ((2 * (4 * hparams->n_embd) / 3 + n_mult - 1) / n_mult) * n_mult;
    return n_ff;
}
...
struct hparams_lora {
    ...
    bool operator!=(const hparams_lora &other) const { return memcmp(this, &other, sizeof(hparams_lora)) != 0; }
};
...
struct _t_model {
    ...
    std::vector<_t_layer> layers;
};

struct llama_model_lora {
    struct ggml_context *ctx = NULL;

    hparams_lora hparams;
    ...
    std::vector<_t_layer_lora> layers;
};

static void init_model(struct _t_model *model) {
...
static void set_param_model(struct _t_model *model) {
    const auto &hparams = model->hparams;
...
static void randomize_model(struct _t_model *model, int seed, float mean, float std, float min, float max) {
    const auto &hparams = model->hparams;
...
static void randomize_model_lora(struct llama_model_lora *model, int seed, float mean, float std, float min, float max) {
    const auto &hparams = model->hparams;
...
static void init_kv_cache(struct lite_cache *cache, struct _t_model *model, int n_batch) {
    const auto &hparams = model->hparams;
...
static bool init_kv_cache_lora(struct lite_cache *cache, struct llama_model_lora *model, int n_batch) {
    const auto &hparams = model->hparams;
...
static struct ggml_tensor *forward(struct _t_model *model, struct lite_cache *cache, struct ggml_context *ctx0, struct ggml_cgraph *gf,
                                   struct ggml_tensor *tokens_input, const int n_tokens, const int n_past) {
    const int N = n_tokens;

    struct lite_cache &kv_self = *cache;
    const auto &hparams        = model->hparams;
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            struct ggml_tensor *Qcur =
                ggml_rope(ctx0, ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wq, cur), n_embd / n_head, n_head, N), KQ_pos, n_rot, 0);
            struct ggml_tensor *Kcur =
                ggml_rope(ctx0, ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wk, cur), n_embd / n_head, n_head, N), KQ_pos, n_rot, 0);
            ...
            // store key and value to memory
            {
                ...
                struct ggml_tensor *Vcur =
                    ggml_cont(ctx0, ggml_transpose(ctx0, ggml_reshape_2d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wv, cur), n_embd, N)));
            ...
            struct ggml_tensor *K =
                ggml_permute(ctx0,
                             ggml_reshape_3d(ctx0, ggml_view_1d(ctx0, kc, (n_past + N) * n_embd, il * n_ctx * ggml_element_size(kc) * n_embd), n_embd / n_head,
                                             n_head, n_past + N),
                             0, 2, 1, 3);
            ...
            struct ggml_tensor *V = ggml_view_3d(ctx0, vc, n_past + N, n_embd / n_head, n_head, n_ctx * ggml_element_size(vc),
                                                 n_ctx * ggml_element_size(vc) * n_embd / n_head, il * n_ctx * ggml_element_size(vc) * n_embd);
...
static struct ggml_tensor *forward_batch(struct _t_model *model, struct lite_cache *cache, struct ggml_context *ctx0, struct ggml_cgraph *gf,
                                         struct ggml_tensor *tokens_input, const int n_tokens, const int n_past, const int n_batch) {
    const int N = n_tokens;
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            struct ggml_tensor *Qcur =
                ggml_rope(ctx0, ggml_reshape_4d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wq, cur), n_embd / n_head, n_head, N, n_batch), KQ_pos, n_rot, 0);
            struct ggml_tensor *Kcur =
                ggml_rope(ctx0, ggml_reshape_4d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wk, cur), n_embd / n_head, n_head, N, n_batch), KQ_pos, n_rot, 0);
            ...
            // store key and value to memory
            {
                ...
                struct ggml_tensor *Vcur =
                    ggml_cont(ctx0, ggml_permute(ctx0, ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wv, cur), n_embd, N, n_batch), 1, 0, 2, 3));
                ...
                kc = ggml_set_2d(ctx0, kc, ggml_reshape_2d(ctx0, Kcur, n_embd * N, n_batch), ggml_element_size(kc) * n_embd * n_ctx,
                                 (ggml_element_size(kc) * n_embd) * (il * n_batch * n_ctx + n_past));
                vc = ggml_set_2d(ctx0, vc, ggml_reshape_2d(ctx0, Vcur, N * n_embd, n_batch), ggml_element_size(vc) * n_ctx * n_embd,
            ...
            struct ggml_tensor *K =
                ggml_permute(ctx0,
                             ggml_reshape_4d(ctx0,
                                             ggml_view_3d(ctx0, kc, n_embd, (n_past + N), n_batch, n_embd * ggml_element_size(kc),
                                                          n_ctx * n_embd * ggml_element_size(kc), il * n_batch * n_ctx * n_embd * ggml_element_size(kc)),
            ...
            struct ggml_tensor *V = ggml_view_4d(ctx0, vc, n_past + N, n_embd / n_head, n_head, n_batch, ggml_element_size(vc) * n_ctx,
                                                 ggml_element_size(vc) * n_ctx * n_embd / n_head, ggml_element_size(vc) * n_ctx * n_embd,
...
static struct ggml_tensor *forward_lora(struct llama_model_lora *model, struct lite_cache *cache, struct ggml_context *ctx0, struct ggml_cgraph *gf,
                                        struct ggml_tensor *tokens_input, const int n_tokens, const int n_past) {
    const int N = n_tokens;
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            struct ggml_tensor *Qcur = ggml_rope(
                ctx0,
                ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wqa, ggml_mul_mat(ctx0, model->layers[il].wqb, cur)), n_embd / n_head, n_head, N),
                KQ_pos, n_rot, 0);
            struct ggml_tensor *Kcur = ggml_rope(
                ctx0,
                ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wka, ggml_mul_mat(ctx0, model->layers[il].wkb, cur)), n_embd / n_head, n_head, N),
                KQ_pos, n_rot, 0);
            ...
            // store key and value to memory
            {
                ...
                struct ggml_tensor *Vcur = ggml_cont(
                    ctx0, ggml_transpose(ctx0, ggml_reshape_2d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wva, ggml_mul_mat(ctx0, model->layers[il].wvb, cur)),
                                                               n_embd, N)));
            ...
            // projection (no bias)
            // cur shape [n_embd,N,1,1]
            cur = ggml_mul_mat(ctx0, model->layers[il].woa, ggml_mul_mat(ctx0, model->layers[il].wob, cur));
        }
    ...
    // lm_head
    // inpL shape [n_vocab,N,1,1]
    inpL = ggml_mul_mat(ctx0, model->outputa, ggml_mul_mat(ctx0, model->outputb, inpL));
...
static void sample_softmax_batch(struct ggml_context *ctx, struct ggml_tensor *logits, struct ggml_tensor *probs, struct ggml_tensor *best_samples) {
    ...
    for (int k = 0; k < n_batch; ++k) {
        struct ggml_tensor *best_samples_k = ggml_view_1d(ctx, best_samples, best_samples->ne[0], k * best_samples->nb[1]);
        struct ggml_tensor *logits_k       = ggml_view_2d(ctx, logits, logits->ne[0], logits->ne[1], logits->nb[1], k * logits->nb[2]);
        struct ggml_tensor *probs_k        = ggml_view_2d(ctx, probs, probs->ne[0], probs->ne[1], probs->nb[1], k * probs->nb[2]);
...
static void get_example_targets_batch(struct ggml_context *ctx, int example_id, struct ggml_tensor *tokens_input, struct ggml_tensor *targets) {
...
static struct ggml_tensor *square_error_loss(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b) {
    // todo: instead of a-b: a[1:]-b[:-1]
    return ggml_sum(ctx, ggml_sqr(ctx, ggml_sub(ctx, a, b)));
}

static struct ggml_tensor *cross_entropy_loss(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b) {
    const float eps = 1e-3f;
    return ggml_sum(
        ctx,
        ggml_neg(ctx, ggml_sum_rows(ctx, ggml_mul(ctx, ggml_soft_max(ctx, a), ggml_log(ctx, ggml_add1(ctx, ggml_soft_max(ctx, b), ggml_new_f32(ctx, eps)))))));
}
...
int Tutor(CLI_params &hparams) {
    ...
    struct _t_model model;
    model.hparams.n_vocab = 8;
    n_ctx                 = 8;  // model.hparams.n_ctx   = 8;
    model.hparams.n_embd  = 32;
    // model.hparams.n_mult  = 2;
    n_head                = 8;  // model.hparams.n_head  = 8;
    model.hparams.n_layer = 1;
    model.hparams.n_rot   = std::min(16u, model.hparams.n_embd / n_head);
    ...
    int n_examples = 256;
    int n_tokens   = n_ctx;  // model.hparams.n_ctx;
    ...
    for (int ex = 0; ex < n_examples; ++ex) {
        ...
        TGraph(nullptr, "lama_tutor", gf, true).__repr__(info, info, e);
