...
hGTensor MixOfModels::Forward(void* ctx, hGTensor cur, hGTensor w) {
#ifdef _TENSOR_G_
    return nullptr;
#else
    int n_vocab = cur->ne[0], n_ctx = cur->ne[1], n_batch = cur->ne[2];
    hGTensor expert = ggml_reshape_2d(ctx, cur, n_vocab, n_ctx * n_batch);
    hGTensor wB     = _repeat(ctx, w, expert);
    gTN(wB, "%s_r", w->name);
...
hGTensor MixOfSwarm::Build(CLI_params& config, void* ctx, hGTensor cur, int flag) {
    hGTensor ouput = nullptr;
#ifdef _TENSOR_G_
#else
    // return cur;
    ...
    hGTensor w_ = ggml_mul_mat(ctx, gat_, ggml_reshape_2d(ctx, cur, N0, nToken));
    if (isSiLU) {  // maybe useful
        w_ = ggml_silu(ctx, w_);
    }
    hGTensor probs = ggml_soft_max(ctx, w_);
    // hGTensor tA = ggml_reshape_2d(ctx,curlogits,n_vocab,n_ctx*n_batch);      //[nVocab, n_ctx*nBatch]
    hGTensor wA = ggml_view_2d(ctx, probs, 1, nToken, ld1, 0);  //[1, n_ctx*nBatch] ne0,ne1,nb1,offset
    //  wA = _repeat(ctx,wA,tA);
    ouput = Forward(ctx, cur, wA);  // ggml_mul(ctx,tA,wA);
    for (auto tB : exs) {
        i++;
        // float *logistB = (float*)(tB->data);    //always nullptr
        hGTensor wB     = ggml_view_2d(ctx, probs, 1, nToken, ld1, offset * i);  // ne0,ne1,nb1,offset
        hGTensor expert = Forward(ctx, tB, wB);
...
hGTensor NLP_AutoRegressive::build_gate(void* ctx, hGTensor cur, hGTensor curlogits, int flag) {
    hGTensor ouput = nullptr;
#ifdef _TENSOR_G_
#else
    bool isRes = true, isSiLU = false;

    int n_vocab = hDictVAE->nClass(), n_batch = config.common.n_batch, n_ctx = config.common.n_ctx, n_embd = config.nEmbed();
    int nWiki = wikis.size(), i = 0;
    CHILD_0909_WIKIS
    size_t N0 = cur->ne[0], ld1 = (nWiki + 1) * cur->nb[0];
    assert(nWiki + 1 == mom.embed2w->ne[1]);

    hGTensor gat_ = mom.embed2w;
    //[nWiki+1, n_tokens] := [n_embed,nWiki+1]x[n_embed,n_tokens]
    hGTensor w_ = ggml_mul_mat(ctx, mom.embed2w, ggml_reshape_2d(ctx, cur, N0, n_ctx * n_batch));
    if (isSiLU) {  // maybe useful
        w_ = ggml_silu(ctx, w_);
    }
    hGTensor probs = ggml_soft_max(ctx, w_);
    ...
    // hGTensor tA = ggml_reshape_2d(ctx,curlogits,n_vocab,n_ctx*n_batch);      //[nVocab, n_ctx*nBatch]
    hGTensor wA = ggml_view_2d(ctx, probs, 1, n_ctx * n_batch, ld1, 0);  //[1, n_ctx*nBatch] ne0,ne1,nb1,offset
    wA->name[0] = '\0';
    ...
    for (auto wiki : wikis) {
        ...
        hGTensor wB = ggml_view_2d(ctx, probs, 1, n_ctx * n_batch, ld1, offset * i);  // ne0,ne1,nb1,offset
        wB->name[0] = '\0';
        gTN(wB, "gate_probs_%d", i);
        hGTensor expert = mom.Forward(ctx, tB, wB);
