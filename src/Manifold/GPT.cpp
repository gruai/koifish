...
static  float random_f32(uint64_t *state) { // random float32 in [0,1)
...
GeneratOnPrompt::GeneratOnPrompt(struct gpt_params&par_,int flag) : params(par_)      {
    ...
    std::mt19937 rng(params.seed);  //42
...
GeneratOnPrompt::GeneratOnPrompt(hWIKI wiki_,Ganglia* hG_,int flag) : gang(hG_), wiki(wiki_){
    // prompt = "when the smoke is going down,";
...
bool GeneratOnPrompt::Init(const std::string& prompt_,int flag){
    ...
    model = lama->lmodel;           assert(model!=nullptr);
    ctx = lama->_ctx;               assert(ctx!=nullptr);
...
int Ganglia::GenSentence(int flag){
    ...
    int genT = 64, nTokens = datas.n_vocab,nB = hparams.common.n_ctx,i;
    ...
    probs.resize(nTokens);
    _INFO("%s:<--- ",__func__);
    for (i = 1; i <= genT; i++) {
        GetOutput(piffle,probs);
        ...
        piffle[i] = next_token;
        _INFO("%s ", wiki->T2STR(next_token).c_str());        fflush(stdout);
    }
    _INFO("--->\n");
}
...
int GeneratOnPrompt::Generate(int nJob,int flag) {
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        if ((int) embd_inp.size() <= n_consumed) {
            ...
            OnAntiPrompt(0x0);
            OnInteractive(n_past,n_consumed,n_remain,0x0);            
        }
