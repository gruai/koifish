...
class TGraph : public std::enable_shared_from_this<TGraph> {
   ...
   protected:
    enum ORDER { LEFT_TO_RIGHT = 0, RIGHT_TO_LEFT, COUNT };
    Fish* hFish = nullptr;
   ...
   public:
    ...
    TGraph(void* ctx_, size_t size_, bool grad_, bool isOnlySymbol_, int flag = 0x0) : size(size_), ctx(ctx_), isOnlySymbol(isOnlySymbol_) {
        assert(0);  // Deprecated
    }
