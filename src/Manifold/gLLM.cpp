...
int Fish::BackwardOnRLS(int iter, int flag) {
    ...
    hFuyou afu   = hRLS->afu;
    afu->Backward(cur);
    /*vector<TaskNode *> curTasks = hRLS->curTasks();
    ...
    SYNC_DEVICE();*/
    if (iter >= config.scheduling.nFuyouWarmup() && hRLS->fuyous.size() > 1) {  // memory
...
int Fish::ForwardOnRLS(int iter, int flag) {
    ...
    if (isAtPhase(LIFE_PHASE::P_EVAL_) || isAtPhase(LIFE_PHASE::P_GENERATE)) {
        if (config.model.ensemble == MODEL_ENSEMBLE::RANDOM_1 && nFuyou > 1) {  //  random ensembling
            branches      = hRLS->fuyous;
            uint32_t pick = rand_coin.RandU32() % hRLS->fuyous.size();
            branches      = {hRLS->fuyous[pick]};
        } else if (config.model.ensemble == MODEL_ENSEMBLE::AGGREGATION && nFuyou > 1) {
            branches      = hRLS->fuyous;
        } else{
