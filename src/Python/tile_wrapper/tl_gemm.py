...
import re
from .tl_utils import Codes2Lines, get_gemm_heuristic_sweep, get_gemm_sweep, tl_clear_cache, Find_kernel_source
...
def gemm_kernel(    M,    N,    K,    block_M,    block_N,    block_K,    num_stages,    thread_num,    enable_rasteration,):
    dtype = T.bfloat16
...
def matmul(M, N, K, block_M, block_N, block_K, num_stages, thread_num, enable_rasteration):
    dtype = T.bfloat16
    accum_dtype = T.float32
...
# @tilelang.jit(out_idx=[-1])
def matmul_transposed_b(M, N, K, block_M, block_N, block_K, num_stages, thread_num, enable_rasteration):
    dtype = T.bfloat16
    accum_dtype = T.float32
    assert (block_M % 16 == 0 and block_N % 16 == 0)    #   Align Shared Memory Accesses
...
def gemm_instance(M, N, K, func):
    if func=="matmul_transposed_b":
        def kernel(block_M,block_N,block_K,num_stages,thread_num,enable_rasteration,    ):        
            return matmul_transposed_b(M, N, K,block_M, block_N, block_K,num_stages, thread_num,enable_rasteration,)
        return kernel
    else:

            # return gemm_kernel(M, N, K,block_M, block_N, block_K,num_stages, thread_num,enable_rasteration,        )
        return None
    return None
    
def run_regression_perf(M, N, K, func, use_autotune: bool = False, with_roller: bool = False, profile_backend: str = "event",):
    if use_autotune:
        kernel = gemm_instance(M, N, K, func)
        if kernel is None:
            return None, None
        autotuner = (
        ...
        config = get_gemm_heuristic_sweep()
...
def GenKernesTable(desc_kernel, kernel_metas, all_codes, M, N, K):    
    for desc, func, TRANS in desc_kernel:
        id = len(kernel_metas)
        title = f"_tl_M{M}_N{N}_K{K}"
        kernel, result = run_regression_perf(M, N, K, func, use_autotune=True)
        if kernel is None:
            continue
        block_M, block_N, block_K = result.config["block_M"],result.config["block_N"],result.config["block_K"]
        # kernel = func(M, N, K, block_M, block_N, block_K)
        # code_0 = kernel.get_kernel_source()
        code_0 = Find_kernel_source(kernel_metas, "best_config.json")
        if code_0 is None or len(code_0)==0:
            assert(0)
