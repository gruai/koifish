...
int DataSample::SelectTo(hDATASET hTo, vector<int> &nEach, bool isMove, int flag) {
    ...
    for (cls = 0; cls < nCls; cls++) {
        ...
        nEach[cls] = MIN(nEach[cls], nz);
        assert(nEach[cls] >= 0);
        nz_0 = MIN(nz_0, nEach[cls]);
        nz_1 = MAX(nz_1, nEach[cls]);
        for (i = 0; i < nz; i++) {
            no = list[i];
            assert(tag[no] == cls);
            if (i < nEach[cls]) {
    ...
    for (i = iS; i < hTo->nMost; i++) hTo->tag[i] = -1;
    nTo = hTo->nSample();
    if (isMove) {
        for (i = iT; i < nData; i++) {
            if (tag[i] >= nCls)
...
FeatData *FeatData::read_json(const string &sPath, int flag) {
    ...
    for (json::iterator it = js.begin(); it != js.end(); ++it, ++i) {
        ...
        double *Y = hData->Y(i);
        for (j = 0; j < nEle - 1; j++) tX[j] = it->at(j);
        label = it->at(nEle - 1);
...
FeatData::FeatData(const string &sPath, int flag) : DataSample_T<float>(), hBase(nullptr), feats(nullptr), mark(nullptr), distri(nullptr), qOrder(false) {
    if (flag == 0x100) {
