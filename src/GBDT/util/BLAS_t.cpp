...
template <>
bool IS_NAN<COMPLEXd>(const COMPLEXd &a) {
    return std::isnan(a.real()) || std::isnan(a.imag());
}
template <>
bool IS_NAN<COMPLEXf>(const COMPLEXf &a) {
    return std::isnan(a.real()) || std::isnan(a.imag());
}

template <>
double ABS_1<float>(const float &a) {
    return fabs(a);
}
template <>
double ABS_1<double>(const double &a) {
    return fabs(a);
}
template <>
double ABS_1<COMPLEXd>(const COMPLEXd &a) {
    return fabs(a.real()) + fabs(a.imag());
}
template <>
double ABS_1<COMPLEXf>(const COMPLEXf &a) {
    return fabs(a.real()) + fabs(a.imag());
}

template <>
double TxTc<COMPLEXd>(const COMPLEXd &a) {
    double re = a.real(), im = a.imag();
    return re * re + im * im;
}
template <>
double TxTc<COMPLEXf>(const COMPLEXf &a) {
    double re = a.real(), im = a.imag();
    return re * re + im * im;
}
...
template <>
double NRM2<double>(const int dim, const double *X) {
    return DNRM2(&dim, X, &inc_1);
}
template <>
double NRM2<float>(const int dim, const float *X) {
    return SNRM2(&dim, X, &inc_1);
}
template <>
double NRM2<COMPLEXd>(const int dim, const COMPLEXd *X) {
    return DZNRM2(&dim, (MKL_Complex16 *)X, &inc_1);
}
template <>
double NRM2<COMPLEXf>(const int dim, const COMPLEXf *X) {
    return SCNRM2(&dim, (MKL_Complex8 *)X, &inc_1);
}
// template <typename T> void SCAL( const int dim,const double alpha,T *X )
template <>
void SCALd<double>(const int dim, const double alpha, double *X) {
    DSCAL(&dim, &alpha, X, &inc_1);
}
template <>
void SCALd<float>(const int dim, const double alpha, float *X) {
    float a = alpha;
    SSCAL(&dim, &a, X, &inc_1);
}
template <>
void SCALd<COMPLEXd>(const int dim, const double alpha, COMPLEXd *X) {
    ZDSCAL(&dim, &alpha, (MKL_Complex16 *)X, &inc_1);
}
template <>
void SCALd<COMPLEXf>(const int dim, const double alpha_, COMPLEXf *X) {
    float alpha = alpha_;
    CSSCAL(&dim, &alpha, (MKL_Complex8 *)X, &inc_1);
}

template <>
void SCAL<COMPLEXd>(const int dim, const COMPLEXd alpha, COMPLEXd *X) {
...
template <>
void COPY<float>(const int dim, const float *X, float *Y) {
    SCOPY(&dim, X, &inc_1, Y, &inc_1);
}
template <>
void COPY<double>(const int dim, const double *X, double *Y) {
    DCOPY(&dim, X, &inc_1, Y, &inc_1);
}
template <>
void COPY<COMPLEXd>(const int dim, const COMPLEXd *X, COMPLEXd *Y) {
    ZCOPY(&dim, (const MKL_Complex16 *)X, &inc_1, (MKL_Complex16 *)Y, &inc_1);
}

template <>
void IMAT_T<COMPLEXf>(const char ordering, size_t rows, size_t cols, COMPLEXf *AB) {
...
template <>
void SWAP<COMPLEXf>(const int dim, COMPLEXf *X, int incx, COMPLEXf *Y, int incy) {
    CSWAP(&dim, (MKL_Complex8 *)X, &incx, (MKL_Complex8 *)Y, &incy);
}

// Y := a*X + Y
template <>
void AXPY<double>(const int dim, const double alpha, const double *X, double *Y) {
    DAXPY(&dim, &alpha, X, &inc_1, Y, &inc_1);
    //	for( int k = 0; k < dim; k++ )	Y[k]+=alpha*X[k];
}
template <>
void AXPY<float>(const int dim, const float alpha, const float *X, float *Y) {
    SAXPY(&dim, &alpha, X, &inc_1, Y, &inc_1);
}
template <>
void AXPY<COMPLEXd>(const int dim, const COMPLEXd alpha, const COMPLEXd *X, COMPLEXd *Y) {
    /*for( int i=0;i<dim;i++ ){
        Y[i] += alpha*X[i];
    }*/
...
template <>
void AXPY<COMPLEXf>(const int dim, const COMPLEXf alpha, const COMPLEXf *X, COMPLEXf *Y) {
    CAXPY(&dim, (const MKL_Complex8 *)(void *)(&alpha), (const MKL_Complex8 *)(void *)X, &inc_1, (MKL_Complex8 *)(void *)Y, &inc_1);
}

// Y := a*X + bY
template <>
void AXPBY<double>(const int dim, const double alpha, const double *X, const double beta, double *Y) {
    DAXPBY(&dim, &alpha, X, &inc_1, &beta, Y, &inc_1);
}
template <>
void AXPBY<float>(const int dim, const float alpha, const float *X, const float beta, float *Y) {
...
template <>
void GEMV<double>(char transa, const int M, const int N, const double alpha, const double *A, const int lda, const double *X, const int incx, const double beta,
...
template <>
void GEAV<C>(char transa, const int M, const int N, const C *A, const int lda, const C *X, C *Y) {  // Y=AX
...
template <>
void GEAV<double>(char transa, const int M, const int N, const double *A, const int lda, const double *X, double *Y) {  // Y=AX
    double one(1.0), zero(0.0);
    DGEMV(&transa, &M, &N, &one, A, &lda, X, &inc_1, &zero, Y, &inc_1);
}

template <>
void GEMM<COMPLEXd>(char transa, char transb, int m, int n, int k, const COMPLEXd *alpha, COMPLEXd *a, int lda, COMPLEXd *b, int ldb, const COMPLEXd *beta,
                    COMPLEXd *c, int ldc) {
    ZGEMM(&transa, &transb, &m, &n, &k, (MKL_Complex16 *)alpha, (MKL_Complex16 *)a, &lda, (MKL_Complex16 *)b, &ldb, (MKL_Complex16 *)beta, (MKL_Complex16 *)c,
          &ldc);
}
template <>
void GEMM<COMPLEXf>(char transa, char transb, int m, int n, int k, const COMPLEXf *alpha, COMPLEXf *a, int lda, COMPLEXf *b, int ldb, const COMPLEXf *beta,
                    COMPLEXf *c, int ldc) {
    CGEMM(&transa, &transb, &m, &n, &k, (MKL_Complex8 *)alpha, (MKL_Complex8 *)a, &lda, (MKL_Complex8 *)b, &ldb, (MKL_Complex8 *)beta, (MKL_Complex8 *)c, &ldc);
}
template <>
void C_GEMM<COMPLEXf>(char transa, char transb, int m, int n, int k, const COMPLEXf *alpha, COMPLEXf *a, int lda, COMPLEXf *b, int ldb, const COMPLEXf *beta,
                      COMPLEXf *c, int ldc) {
    CBLAS_TRANSPOSE ta = CblasNoTrans, tb = CblasNoTrans;
    assert(ldc >= MAX(1, n));
    cblas_cgemm(CblasRowMajor, ta, tb, m, n, k, (MKL_Complex8 *)alpha, (MKL_Complex8 *)a, lda, (MKL_Complex8 *)b, ldb, (MKL_Complex8 *)beta, (MKL_Complex8 *)c,
                ldc);
}

template <>
void GEMM<double>(char transa, char transb, int m, int n, int k, const double *alpha, double *a, int lda, double *b, int ldb, const double *beta, double *c,
                  int ldc) {
    DGEMM(&transa, &transb, &m, &n, &k, alpha, a, &lda, b, &ldb, beta, c, &ldc);
}
template <>
void GEMM<float>(char transa, char transb, int m, int n, int k, const float *alpha, float *a, int lda, float *b, int ldb, const float *beta, float *c,
                 int ldc) {
...
/*
template<>
void COO_MM_t<double>( char transa,  int m, int n, int k, double *alpha, double *a, int *rowA,int*colA, int nnz,double *b, int ldb, double *beta, double *c, int
ldc )	{ char matdescra[6]={'g','l','n','f'}; double *temp=nullptr; if( 1 )	{ assert( ldc==m ); temp=new double[m*n];	memcpy( temp,c,sizeof(double)*m*n );
        if( transa==charT )
...
template <>
void LARNV_seed<double>(const int dim, double *X, int dist, int seed) {
...
template <>
void LARNV_seed<float>(const int dim, float *X, int dist, int seed) {
    int iseed[4] = {1, 3, 5, 1};
    for (int i = 0; i < 3; i++) iseed[i] = abs(dim * i + seed) % 4095;
...
template <>
void LARNV<double>(const int dim, double *X, int dist) {
...
template <>
void LARNV<float>(const int dim, float *X, int dist) {
    int iseed[4] = {1, 3, 5, 1}, idist = dist;
    for (int i = 0; i < 3; i++) iseed[i] = abs(dim * i + 1) % 4095;
...
template <>
void LARNV<COMPLEXd>(const int dim, COMPLEXd *X, int seed) {
...
template <>
void LARNV<COMPLEXf>(const int dim, COMPLEXf *X, int seed) {
    // 2: real and imaginary parts each uniform (-1,1)
    // 4: uniformly distributed on the disc abs(z) < 1		5: uniformly distributed on the circle abs(z) = 1
    int iseed[4] = {1, seed, 5, 7}, idist = 2;
...
template <>
int GEQR_p<COMPLEXd>(int m, int n, COMPLEXd *A, int ldA, COMPLEXd *Q, int *jpvt, int flag) {
    ...
    if (info == 0) {
        ...
        for (int i = 0; i < n; i++) {
            pivot = A[i * m + i];
            assert(std::abs(pivot) <= std::abs(pivot_0));
            pivot_0 = pivot;
        }
    } else {
        assert(0);
...
template <>
int GEQR_p<COMPLEXf>(int m, int n, COMPLEXf *A, int ldA, COMPLEXf *Q, int *jpvt, int flag) {
...
*/
template <>
double vCOS<double>(const int dim, const double *a, const double *b, int flag) {
...
template <>
void vEXP<double>(const int dim, double *Z) {
#ifdef MKL_LIB
    vdExp(dim, Z, Z);
#else
    for (int i = 0; i < dim; i++) {
...
template <>
void vEXP<float>(const int dim, float *Z) {
#ifdef MKL_LIB
    vsExp(dim, Z, Z);
#else
    for (int i = 0; i < dim; i++) {
...
template <>
void GESVD<double>(const int m, const int n, double *a, double *sval, double *u, double *v, int lda, int ldu, int ldv) {
...
template <>
void GESVD<COMPLEXf>(const int m, const int n, COMPLEXf *a, COMPLEXf *sval_, COMPLEXf *u, COMPLEXf *v, int lda, int ldu, int ldv) {
    ...
    if (ldv == 0)
        ldv = n;
    int iRet = LAPACKE_cgesvd(LAPACK_COL_MAJOR, charA, charA, m, n, (MKL_Complex8 *)a, lda, sval, (MKL_Complex8 *)u, ldu, (MKL_Complex8 *)v, ldv, superb);
    delete[] superb;
    if (iRet != 0)
        throw("GESVD failed!!!");
    ;
...
template <>
void GESOP<float>(int dim, int n, float *X, double *dSop, int flag) {
    int i, inc = 1;
...
#endif
    if (old != nullptr)
        memcpy(old, X, sizeof(double) * dim * dim);
...
template <>
int POTRF<COMPLEXd>(const char uplo, int dim, COMPLEXd *X, int ldX) {
    int info = 0;
    ZPOTRF(&uplo, &dim, (MKL_Complex16 *)X, &ldX, &info);
    return info;
}
template <>
int POTRF<COMPLEXf>(const char uplo, int dim, COMPLEXf *X, int ldX) {
    int info = 0;
...
template <>
int GETRF(int dim, COMPLEXf *mA, int lda, int *ipiv, int flag) {
...
template <>
int GETRS(int dim, COMPLEXf *mA, int lda, int *ipiv, int nRhs, COMPLEXf *b, int ldb, int flag) {
    lapack_int iRet;
...
template <>
int GETRF_r(int dim, COMPLEXf *mA, int lda, int *ipiv, int flag) {
    assert(ipiv != nullptr && mA != nullptr);
    /*	[MKL]		A = P*L*U,
        L is lower triangular with unit diagonal elements U is upper triangular The routine uses partial pivoting, with row interchanges.
        [GSS]		A =  L*U,		ROW-MAJOR
...
template <>
int GETRS_r(int dim, COMPLEXf *mA, int lda, int *ipiv, int nRhs, COMPLEXf *b, int ldb, int flag) {
    assert(ipiv != nullptr && mA != nullptr);
    lapack_int iRet = LAPACKE_cgetrs(LAPACK_ROW_MAJOR, charN, dim, nRhs, (MKL_Complex8 *)mA, lda, ipiv, (MKL_Complex8 *)b, ldb);
    return iRet;
}
template <>
int GETRF_r(int dim, COMPLEXd *mA, int lda, int *ipiv, int flag) {
    assert(ipiv != nullptr && mA != nullptr);
...
template <>
int GETRS_r(int dim, COMPLEXd *mA, int lda, int *ipiv, int nRhs, COMPLEXd *b, int ldb, int flag) {
    assert(ipiv != nullptr && mA != nullptr);
