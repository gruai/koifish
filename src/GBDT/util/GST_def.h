...
#ifdef WIN32
#include <assert.h>
#define GST_VERIFY(isTrue, info)            \
    {                                       \
        if (!(isTrue)) {                    \
            GST_THROW(info); /*assert(0);*/ \
        }                                   \
    }
#define LOGP printf
#else
...
#define G_RC_NORMAL(r, r_0, r_1) \
    {                            \
        (r) = MAX2((r), (r_0));  \
        (r) = MIN2((r), (r_1));  \
    }

#define G_MINMAX(y, y0, y1)     \
    {                           \
        (y1) = MAX2((y), (y1)); \
        (y0) = MIN2((y), (y0)); \
    }
...
#define FWRIT_I64(num, fp) \
    { GST_VERIFY(fwrite(&(num), sizeof(int64_t), 1, (fp)) == 1, ""); }

#define FREAD_i(num, fp) \
    { GST_VERIFY(fread(&(num), sizeof(int), 1, (fp)) == 1, ""); }
#define FWRIT_i(num, fp) \
    { GST_VERIFY(fwrite(&(num), sizeof(int), 1, (fp)) == 1, ""); }

#define FREAD_f(num, fp) \
    { GST_VERIFY(fread(&(num), sizeof(float), 1, (fp)) == 1, ""); }
#define FWRIT_f(num, fp) \
    { GST_VERIFY(fwrite(&(num), sizeof(float), 1, (fp)) == 1, ""); }
#define FREAD_d(num, fp) \
    { GST_VERIFY(fread(&(num), sizeof(double), 1, (fp)) == 1, ""); }
#define FWRIT_d(num, fp) \
    { GST_VERIFY(fwrite(&(num), sizeof(double), 1, (fp)) == 1, ""); }

#define FREAD_arr(arr, size, count, fp) \
    { GST_VERIFY(fread((arr), (size_t)(size), (size_t)(count), (fp)) == ((size_t)count), ""); }
