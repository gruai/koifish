...
class FeatVec_LOSS {
   protected:
    ...
    /*
        �������⣬Ϊ������汾����
        1 outlierֻ�Ա��˺���һ��������
        2 rou = HT_lambda_.At(y1[i] - y1[samp])	д����
    */
    template <typename Tx>
    ...
    void Lambda_1(int flag = 0x0) {
        ...
        assert(nz == dim);
        std::mt19937 rand_g(42);  
        std::shuffle(ids + nOutlier, ids + dim, rand_g);
        // std::random_shuffle(ids + nOutlier, ids + dim);  // deprecated
        nMost = MIN2((dim - nOutlier) / nOutlier, 1000);
        nz    = nOutlier;
        for (auto samp : outliers) {
            for (i = nz; i < nz + nMost; i++) {
                id = ids[i];
                // rou = HT_lambda_.At(y1[samp] - y1[id]);
                rou  = HT_lambda_.At(y1[id] - y1[samp]);  // Ч�����Ը��ã���������
                grad = -rou;
                pDown[samp] -= grad;  // down=-grad
                pDown[id] += grad;
                a = sigma * rou * (1 - rou);
    ...
    template <typename Tx>
    void WeightOnMarginal_2(FeatsOnFold* hData_, int round, int flag) {  // �������ԣ����adaptive lr ������Ч����
        ...
        double N_1 = exp(decrimi_2.N_1);
        N_1        = N_1 / (1 + N_1);
        double P_0 = exp(decrimi_2.P_0);
        P_0        = P_0 / (1 + P_0);
        // #pragma omp parallel for schedule(static,1)
        for (int thread = 0; thread < num_threads; thread++) {
            size_t start = thread * step, end = MIN2(start + step, dim), i;
            for (i = start; i < end; i++) {
                ...
                w0 = MIN2(w0, samp_weight[i]);
                w1 = MAX2(w1, samp_weight[i]);
            };
        }
        if (round % 100 == 0) {
...
#pragma omp parallel for schedule(static, 1) reduction(+ : a2, sum, label_sum, sumGH)
            for (int thread = 0; thread < num_threads; thread++) {
                ...
                for (i = start; i < end; i++) {
                    ...
                    pDown[i] = a = -(sig - label[i]);
                    vHess[i]     = sig * (1 - sig);
                    if (samp_weight != nullptr) {
                        pDown[i] *= samp_weight[i];
                        vHess[i] *= samp_weight[i];
                    }
                    a2 += a * a;
                    sum += a;
...
#pragma omp parallel for schedule(static, 1) reduction(+ : a2, a1)
                    ...
                    DOWN_sum_2 = a2;
                    DOWN_sum_1 = a1;
                    if (samp_weight != nullptr && pDown != nullptr) {  // ���Լ������Ȩ
...
#pragma omp parallel for schedule(static, 1) reduction(+ : a2, a1)
            ...
            err_mae  = a1;
            // sum = NRM2(dim, vResi);
            // �μ�loss = PointWiseLossCalculator::AverageLoss(sum_loss, sum_weights_)��L2Metric���
            err_l2        = err_rmse / dim;
   ...
   public:
    std::vector<tpDOWN> down, resi, hessian, sample_down, sample_hessian;  // negative_gradient,�Ƿ��½���LOSS�ж�
    ...
    tpDOWN* GetDownDirection() { return VECTOR2ARR(down); }
    tpDOWN* GetDeltaStep() { return VECTOR2ARR(delta_step); }
    tpDOWN* GetSampleDown() { return VECTOR2ARR(sample_down); }
    int* Tag() {  // �������ڷ�������
        CLASS_VEC* cls_vec = dynamic_cast<CLASS_VEC*>(y);
        assert(cls_vec != nullptr);
        return cls_vec->arr();
    }

    size_t size() const { return resi.size(); }
    ...
    void Init_T(const FeatsOnFold* hData_, int _len, size_t x, int rnd_seed, int flag) {
        ...
        y       = new FeatVec_T<Ty>(hData_, _len, 0, "loss");
        predict = new FeatVec_T<Ty>(hData_, _len, 0, "predict");
    ...
    double Adaptive_LR(MT_BiSplit* hBlit, bool isDelta_, int flag = 0x0) {
        ...
        // min_err = -DBL_MAX;
        FeatVec_T<Tx>* hY = dynamic_cast<FeatVec_T<Tx>*>(y);
        assert(hY != nullptr);
