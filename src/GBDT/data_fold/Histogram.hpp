...
typedef enum { BY_VALUE = 0x0, BY_DENSITY } SPLIT_HISTOGRAM;
...
class FRUIT {
   public:
    ...
    int best_feat_id             = -1;
    const HistoGRAM* histo_refer = nullptr;   // ��ָ�򣬲���ɾ��
    HISTO_BIN bin_S0, bin_S1;                 // �б仯���Ƚ�Σ����
    double split_0, adaptive_thrsh, split_1;  // ��ָ��binSplit�м䣬���Ա���		splitonYҲ��Ҫ���ֵ
    ...
    // char buf[256] = { 0 };		//https://stackoverflow.com/questions/4523497/typedef-fixed-length-array	Arrays can't be passed as function parameters by
    // value in C. double lft_impuri,rgt_impuri;		//Ϊ�˵���
    virtual ~FRUIT();
...
class Distribution;
class MINI_HISTO : public HistoGRAM {
   protected:
    int p0 = 0, p1 = -1;
    HistoGRAM* rawHisto  = nullptr;
    FeatsOnFold* hData   = nullptr;
    Distribution* distri = nullptr;
    float impuri         = 0;

   public:
    MINI_HISTO(Distribution* distri, int p0_, int p1_, int flag = 0x0);
    virtual ~MINI_HISTO() { ; }

    class compare {
       public:
        bool operator()(shared_ptr<MINI_HISTO> n0, shared_ptr<MINI_HISTO> n1) const {
            // return n0->gain_train<n1->gain_train;
            return n0->impuri < n1->impuri;
        }
    };
    shared_ptr<MINI_HISTO> left = nullptr, rigt = nullptr;

    virtual bool Split_v2(int minSet, int flag = 0x0);

    virtual float T_left(int flag = 0x0);
};
