...
class FeatVec_Q : public FeatVec_T<tpQUANTI> {
    ...
    */
    virtual void UpdateHisto(const FeatsOnFold* hData_, bool isOnY, bool isFirst, int flag = 0x0) {
        ...
        if (nSamp != hData_->nSample()) {  // ȷʵ�п��� 1 merge_right
    ...
    */
        ...
        if (hessian == nullptr) {
            ...
            {                                       // ��Ҫ��ʱ��ƿ��
                ...
                for (i = 0; i < nSamp4; i += 4) {
                    ...
                    pB3->G_sum -= a3;
                    pB0->H_sum += hessian[i];
                    pB1->H_sum += hessian[i + 1];
                    pB2->H_sum += hessian[i + 2];
                    pB3->H_sum += hessian[i + 3];
                    pB0->nz++;
                    pB1->nz++;
                    pB2->nz++;
                    pB3->nz++;
    ...
    virtual void Samp2Histo_null_hessian(const FeatsOnFold* hData_, const SAMP_SET& samp_set, HistoGRAM* histo, int nMostBin,
                                         const tpSAMP_ID* samps4quanti = nullptr, int flag = 0x0) const {
        ...
        for (i = 0; i < nSamp_LD; i += LD) {
            ...
            if (down != nullptr) {
				...
				pBins[p0].G_sum = 1.0,	pBins[p1].G_sum = 1.0,	pBins[p2].G_sum = 1.0,	pBins[p3].G_sum = 1.0;
			}
