...
/*
�Ƚ�΢������ݽṹ
1 train,test�����ò�ͬ��init
2 ���ļ�����
3 ��Ҫ��INIT_SCOREת����leaf->down_step
...
class Feat_Importance {
   ...
   public:
    ...
    vector<double> gain_sum;  // Friedman  �ڵ����ʱ����Խ�󣬸ýڵ��Ӧ����������Ҫ��Խ��
...
class FeatsOnFold {
   ...
   private:
    tpDOWN* down = nullptr;  // negative_gradient	��ʱ����
   protected:
   ...
   public:
    ...
    LiteBOM_Config config;  // configuration paramters
                            // treeÿ��node����samp_set����Ӱ��data
                            // SAMP_SET samp_set;
                            // label or target values of Y��Only one,���Y��Ȼ��Ҫ���ģ����ѵ����
    ...
    void* GetSampleValues() const {  // ��֧�ֲ��У�����
        return buffer.samp_values;
    }

    /**/
    bool atTrainTask() const { return config.task == LiteBOM_Config::kTrain; }
    bool atPredictTask() const { return config.task == LiteBOM_Config::kPredict; }
    bool isTrain() const { return BIT_TEST(dType, FeatsOnFold::DF_TRAIN); }
    bool isEval() const { return BIT_TEST(dType, FeatsOnFold::DF_EVAL); }
    // �ڲ����ݱ��뱣�ֲ��䣡����
    bool isMerge() const { return BIT_TEST(dType, FeatsOnFold::DF_MERGE); }
    ...
    int *info = nullptr, *permut = nullptr;  // ������ǰ�İ汾
    int nCls = 0;                            // �������ڷ�������
    ...
    virtual void Reshape(int flag = 0x0) { throw "FeatsOnFold::Reshape is ..."; }
    virtual void Shuffle(int flag = 0x0) { throw "FeatsOnFold::Reshape is ..."; }
    size_t nSample(bool isCheck = false) const {
        if (isCheck) {  // ��ʱnSample<nMost�����������趨nMost
                        // size_t i;
                        // for (i = 0; i < nMost ; i++)					;
        }
        return nMost;
    ...
    FeatVector* Feat(const string& name) { return nullptr; }

    float Quant(int nQuant, typNUMBER tpGama, floatGama* gama, hBITARR quant_data, const std::string& desc, int flag = 0x0);
...
#pragma omp parallel for schedule(static, 1)
        for (int thread = 0; thread < num_threads; thread++) {
            ...
            for (t = start; t < end; t++) {
                ...
                while (no != -1) {
                    if (left[no] == -1) {
                        delta_step[t] = thrsh_step[no];  // Adpative_LRҲ��Ҫ����Ϣ
                        break;
                    } else {
                        assert(rigt[no] != -1);
...
typedef std::shared_ptr<FeatsOnFold> hTabularData;
...
class FeatVec_T : public FeatVector {
   ...
   public:
    ...
    void Set_bf16(size_t len, bf16* src, int flag = 0x0) override {
        size_t szT = sizeof(Tx);
        if (len != size())
            throw "FeatVec_T::Set len mismatch!!!";
    ...
    virtual void Set(double a, int flag = 0x0) {
        size_t len = size(), i;
        Tx* x_     = arr();
        for (i = 0; i < len; i++) x_[i] = a;
    }
    virtual void Set(size_t pos, double a, int flag = 0x0) { val[pos] = (Tx)a; }
    /*virtual void Clear(int flag = 0x0) {
        val.clear();
    }*/
    virtual void Empty(int flag = 0x0) { memset(val, 0, size() * sizeof(Tx)); }
    ...
    virtual inline void* pValue_AtSamp(const size_t& samp, int flag = 0x0) {
        ...
        if (samp < 0 || samp >= size()) {
            printf("!!!pValue_AtSamp!!! samp=%ld,size=%ld", samp, size());
            // throw "!!!pValue_AtSamp!!!";
            return val + size() - 1;
        }
        return val + samp;
    ...
    // �μ�MT_BiSplit::Observation_AtSamp(FeatsOnFold *hData_, int flag)
    ...
    /*
        2/26/2019	Adaptive Learning_rate��Ҫ�������(��eval����,��Ӵ�overfit)
    */
    virtual void Update_step(FeatsOnFold* hData_, MT_BiSplit* hBlit, int flag = 0x0) {
        ...
        if (isAdaptive) {
            ...
            FeatVec_T<Tx>* hY = dynamic_cast<FeatVec_T<Tx>*>(hY0);
            assert(hY != nullptr);
            ...
            for (loop = 0; loop < nLoop; loop++) {
                ...
                for (err = 0, i = 0; i < nSamp; i++) {
                    samp = samps[i];
                    a    = val[samp] + (Tx)(step)-target[samp];  //
                    err += a * a;
                }
                err = sqrt(err / nSamp);
                if (err < min_err) {
    ...
    virtual void Update_regression(FeatsOnFold* hData_, MT_BiSplit* hBlit, tpY* target, int flag = 0x0) {
        ...
        if (regress == nullptr && histo == nullptr)
            throw "Update_regression regress=nullptr!!!";
        for (i = 0; i < nSamp; i++) {
    ...
    virtual inline int left_rigt(const void* pVal, const ARR_TREE* arr_tree, int no, int flag = 0x0) {
        ...
        if (fold_map == nullptr) {
            if (IS_NAN_INF(val_t)) {
                ...
                int child = val_t < (Tx)(arr_tree->thrsh_step[no]) ? lft : rgt;
#ifdef _DEBUG
                if (flag == 0x1000000) {
    ...
    void _core_isY_(bool isQuanti, const tpSAMP_ID samp, const tpFOLD* mapFolds, int pos, tpSAMP_ID* left, G_INT_64& nLeft, tpSAMP_ID* rigt, G_INT_64& nRigt,
                    int flag) {
    ...
    virtual void SplitOn(FeatsOnFold* hData_, MT_BiSplit* hBlit, int flag = 0x0) {
        ...
        /*
            ����������	12/19/2018	cys
            1/11/2019	����train��histo->AtBin_��������test
        */
        ...
        if (hBlit->fruit->isY) {
            ...
            lSet                   = hBlit->samp_set;
            rSet                   = hBlit->samp_set;  // ֱ�Ӹ��Ƹ��ڵ��һЩ����
            lSet.isRef             = true;
            rSet.isRef             = true;
            const HistoGRAM* histo = hBlit->fruit->histo_refer;
            int num_threads        = OMP_FOR_STATIC_1(nSamp, step);
...
#pragma omp parallel for schedule(static, 1)
            for (int th_ = 0; th_ < num_threads; th_++) {
                ...
                for (i = start; i < end; i++) {
                    ...
                    _core_isY_(isQuanti, samp, mapFolds, (int)(samp_val[i]), samps + start, nL, rigt + start, nR, 0x0);
                    continue;
        ...
        /*else if (hBlit->fruit->split_by==BY_DENSITY) {
            ...
            lSet = hBlit->samp_set;		rSet = hBlit->samp_set;	//ֱ�Ӹ��Ƹ��ڵ��һЩ����
    ...
    */
    virtual void InitDistri(const FeatsOnFold* hFold, Distribution* tDistri, const SAMP_SET* samp_set, bool isGenHisto, int flag) {
        ...
        if (tDistri == nullptr) {  // Y in train; feats in valid and eavl
            // �μ�ExploreDA::AddDistri(const PY_COLUMN*PY, int id, int flag)
    ...
    void CategoryAtEDA_(ExploreDA* edaX, const vector<tpSAMP_ID>& idx, tpQUANTI* quanti, int nMostBin, const FeatsOnFold* hData_, int flag) {
        ...
        while (i_0 < nA) {
            ...
            if (myD_->mapCategory.find(key) == failed) {
                quanti[pos] = BIN_1 - 1;  // ���ٵ�fail_match	Ҳ�����ؽ���׼ȷ��
    ...
    */
    ...
    void QuantiAtEDA_(ExploreDA* edaX, tpQUANTI* quanti, int nMostBin, const FeatsOnFold* hData_, int flag) {
        ...
        if (myD_->sortedA.size() > 0 && isSameSorted) {
            idx = myD_->sortedA;
            for (i = 0; i < nSamp_; i++) quanti[i] = NNA;
        } else {
        ...
        if (isCategory()) {
            ...
            const HistoGRAM* histo = distri->histo;
            ...
            while (i_0 < nA) {
                ...
                if (val[pos] < distri->vMin || val[pos] > distri->vMax) {  // ȷʵ�п���
                    if (isSameSorted) {
                        quanti[pos] = NNA;
                        i_0++;
                    } else {  // ǿ���Ŷ�
    ...
    /*
        v0.1	cys
            11/18/2025
    */
    float Quant_RTN(int bits, typNUMBER tpGama, void* gama, hBITARR quanti, const FeatsOnFold* hData_, int flag) override {
        // myDistri()->Dump(id, false, flag);
        assert(bits > 0 && bits <= 8);
        int nQuant = 0x1 << bits, NNA = nQuant + 1;
        const Distribution* distri = myDistri();  // hData_->edaX->GetDistri(id);
        size_t nSamp_              = size(), pos;
        int i, i_0 = 0, cur = 0, noBin = 0, nzHisto = 0, nz = 0;
        BIT_8 qid = 0;
        vector<tpSAMP_ID> idx;

        bool isSameSorted = true, isNextQ = false;
        sorted_idx(idx);
        size_t nA = idx.size();
        Tx a0 = val[idx[0]], a1 = val[idx[nA - 1]];
        double v0 = a0, v1, v2 = a1, thrsh = a0 + (a1 - a0) / nQuant, sum = 0, err = 0, a, len2 = 0;
        assert(a0 <= a1);
        // thrsh                  = (int)CEIL_DIV(nA, nQuant);      //equal-size of n
        const HistoGRAM* histo = distri->histo;
        assert(histo != nullptr);
        noBin = 0;                                     // v1 = vThrsh[noBin + 1];
        v1    = distri->binFeatas[noBin + 1].split_F;  // bins[noBin + 1].split_F;
        ...
        while (cur < nA) {
            ...
            isNextQ = val[pos] > thrsh;  // equal-size of value
            //	isNextQ = nz >= thrsh;  // equal-size of n
            if (isNextQ || cur == nA) {
                float average = sum / nz;
                if (nz == 0)  // no element in this quant grid
                    average = 0.0;
                // if(qid==14)
                //     DEBUG_HERE;
                Float2T(tpGama, gama, qid, average);  // gama[id] = average
                for (i = i_0; i < cur; i++) {
                    a = val[idx[i]], len2 += a * a;
                    a = fabs(average - a);
                    err += a * a;
                }
                nz = 0, sum = 0, qid++;
                i_0   = cur;
                thrsh = a0 + (a1 - a0) / nQuant * (qid + 1);
            }
        ...
        while (qid < nQuant) {  // so strange!
            Float2T(tpGama, gama, qid, 0.0f);
            qid++;
        }
        // char typ = ' ';
    ...
    /*
        v0.1	cys
            11/18/2025
    */
    float Quant_MITREE(int bits, typNUMBER tpGama, void* gama, hBITARR quanti, int flag) {
        // myDistri()->Dump(id, false, flag);
        assert(bits > 0 && bits <= 8);
        int nQuant = 0x1 << bits, NNA = nQuant + 1;
        assert(miniTree.size() == nQuant);
        std::vector<hMIHISTO> histos;
        while (!miniTree.empty()) {  // std::priority_queueis a container adapter, it does not allow iteration​ over its elements because:
            histos.push_back(miniTree.top());
            miniTree.pop();
        }
        std::sort(histos.begin(), histos.end(), [](const hMIHISTO& a, const hMIHISTO& b) { return a->T_left() < b->T_left(); });
        ...
        size_t nA = idx.size();
        Tx a0 = val[idx[0]], a1 = val[idx[nA - 1]];
        double thrsh = histos[1]->T_left(), v0 = histos[0]->T_left(), sum = 0, err = 0, a, len2 = 0;
        if (flag == 0x100)
            thrsh = a0 + (a1 - a0) / nQuant;
        assert(a0 <= a1);
        // thrsh                  = (int)CEIL_DIV(nA, nQuant);      //equal-size of n
        const HistoGRAM* histo = distri->histo;
        assert(histo != nullptr);
        ...
        while (cur < nA) {
            ...
            if (isNextQ || cur == nA) {
                ...
                // _INFO("%d %.4g %.4g \n ", nz, average, thrsh);
                nz = 0, sum = 0, qid++;
                thrsh = qid < nQuant - 1 ? histos[qid + 1]->T_left() : a1;
                if (flag == 0x100)
                    thrsh = a0 + (a1 - a0) / nQuant * (qid + 1);
                i_0 = cur;
            }
