			...
			if (isAdaptive) {
				...
				for (loop = 0; loop < nLoop; loop++) {
					...
					for (err=0,i = 0; i<nSamp; i++) {
						samp = samps[i];
						a = val[samp] + step-target[samp];		//��Ӧ��UpdateResi
						err += a*a;
					}					
					err = sqrt(err/nSamp);
					if (err < min_err) {
...
#pragma omp parallel for schedule(static,1)
		...
		*/
		...
		//�μ�Distribution::STA_at����Ҫ��������		8/20/2019
			...
			if (isCategory()) {
				...
				while (i_0 < nA) {
					...
					if (myDistri->mapCategory.find(key) == failed) {
						...
						quanti[pos] = distri->histo->nBins - 1;		//���ٵ�fail_match	Ҳ�����ؽ���׼ȷ��
						//quanti[pos] = 0;
						nFailed++;
					}else
