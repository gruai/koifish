...
class LiteBOM_Config {
   public:
    ...
    LiteBOM_Config(int _num_trees, int _max_leaf, double _min_gain, double _l2_reg, std::string _loss, double _learning_rate, std::string _eval_metric,
                   int _num_classes, int _num_threads,
                   // int _max_bin,
                   double _min_sum_hessian_in_leaf, int _num_vars, std::string _grow_by, std::string _leaf_type, int _max_level, int _verbose,
                   double _sparse_threshold, string _boosting_type, double _goss_alpha, double _goss_beta)
    ...
    void dump(int type = 0x0) {
        ...
        printf(
            ...
            objective.c_str(), eval_metric.c_str(), leaf_optimal.c_str(), init_scor.c_str(), max_depth, lambda_l2, lambda_Feat,
            eda_NA == -1  ? "OFF"
            : eda_NA == 0 ? "0"
                          : "Other",
            eda_Normal == 0 ? "OFF" : "Gaussian", rElitism, T_iterrefine, split_refine, nMostPrune,
            node_task == 0   ? "split_X"
            : node_task == 1 ? "split_Y"
                             : "REGRESS_X",
            isDebug_1 ? "Debug_1" : "Debug_0", nMostSalp4bins, histo_alg.c_str());
    }
...
#pragma region Core Parameters
    ...
    // [doc-only]
    // type = enum
    // options = regression, regression_l1, huber, fair, poisson, quantile, mape, gammma, tweedie, binary, multiclass, multiclassova, xentropy, xentlambda,
    // lambdarank alias = objective_type, app, application desc = regression application descl2 = ``regression_l2``, L2 loss, aliases: ``regression``,
    // ``mean_squared_error``, ``mse``, ``l2_root``, ``root_mean_squared_error``, ``rmse`` descl2 = ``regression_l1``, L1 loss, aliases:
    // ``mean_absolute_error``, ``mae`` descl2 = ``huber``, `Huber loss <https://en.wikipedia.org/wiki/Huber_loss>`__ descl2 = ``fair``, `Fair loss
    // <https://www.kaggle.com/c/allstate-claims-severity/discussion/24520>`__ descl2 = ``poisson``, `Poisson regression
    // <https://en.wikipedia.org/wiki/Poisson_regression>`__ descl2 = ``quantile``, `Quantile regression <https://en.wikipedia.org/wiki/Quantile_regression>`__
    ...
    // descl2 = ``gamma``, Gamma regression with log-link. It might be useful, e.g., for modeling insurance claims severity, or for any target that might be
    // `gamma-distributed <https://en.wikipedia.org/wiki/Gamma_distribution#Applications>`__ descl2 = ``tweedie``, Tweedie regression with log-link. It might be
    // useful, e.g., for modeling total loss in insurance, or for any target that might be `tweedie-distributed
    // <https://en.wikipedia.org/wiki/Tweedie_distribution#Applications>`__ desc = ``binary``, binary `log loss <https://en.wikipedia.org/wiki/Cross_entropy>`__
    // classification (or logistic regression). Requires labels in {0, 1}; see ``xentropy`` for general probability labels in [0, 1] desc = multi-class
    // classification application descl2 = ``multiclass``, `softmax <https://en.wikipedia.org/wiki/Softmax_function>`__ objective function, aliases: ``softmax``
    ...
    // ``multiclass_ova``, ``ova``, ``ovr`` descl2 = ``num_class`` should be set as well desc = cross-entropy application descl2 = ``xentropy``, objective
    // function for cross-entropy (with optional linear weights), aliases: ``cross_entropy`` descl2 = ``xentlambda``, alternative parameterization of
    // cross-entropy, aliases: ``cross_entropy_lambda`` descl2 = label is anything in interval [0, 1] desc = ``lambdarank``, `lambdarank
    // <https://papers.nips.cc/paper/2971-learning-to-rank-with-nonsmooth-cost-functions.pdf>`__ application descl2 = label should be ``int`` type in lambdarank
    // tasks, and larger number represents the higher relevance (e.g. 0:bad, 1:fair, 2:good, 3:perfect) descl2 = `label_gain <#objective-parameters>`__ can be
    // used to set the gain (weight) of ``int`` label descl2 = all values in ``label`` must be smaller than number of elements in ``label_gain``
...
#pragma region Learning Control Parameters
    ...
    // desc = you need to specify all features in order. For example, ``mc=-1,0,1`` means decreasing for 1st feature, non-constraint for 2nd feature and
    // increasing for the 3rd feature
