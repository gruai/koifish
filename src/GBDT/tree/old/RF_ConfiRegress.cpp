...
void RF_ConfiRegress::AfterTrain(int cas, int nMulti, int flag) { return; }
...
hBLIT RF_ConfiRegress::GetBlit(WeakLearner *hWeak, int flag) {
    ...
    double mean, devia, sigma = 0, sigmaL, sigmaR;
    STA_distribute(nSamp, confi, mean, devia);
    sigma = devia * devia * nSamp;
    for (i = 0; i < nRand; i++) {  // Ԥ�����ɣ�������̵߳Ĳ�ȷ����
        do {
    ...
    for (i = 0; i < nRand; i++) {
        ...
        for each (F4NO *fn in hWeak->samps) {
            ...
            if (fu[no] - fv[no] <= thrsh) {
                //	if( fu[no]<=thrsh ){
                aL(nLeft, 0) = 1.0;
                aL(nLeft, 1) = fu[no] - fv[no];
                rhsL(nLeft)  = a;
                nLeft++;
            } else {
                aR(nRight, 0) = 1.0;
                aR(nRight, 1) = fu[no] - fv[no];
                rhsR(nRight)  = a;
                nRight++;
            }
        }
        ...
        g      = sigma - sigmaL - sigmaR;
        assert(g > -FLT_EPSILON);
        if (g > hBlit->gain) {
#pragma omp critical
            if (g > hBlit->gain) {
                hBlit->id    = iU;
...
RF_ConfiRegress::RF_ConfiRegress(RF_ShapeRegress *hReg_) : hRegress(hReg_) {
    ...
    isToCPP = false;
    index   = hReg_->index;
    GST_VERIFY(index == ShapeBMPfold::FOLD_MAP, "RF_ConfiRegress");
...
int RF_ConfiRegress::Train(string sTitle, int cas, int flag) {
    ...
    printf("\n********* ConfiRegress::%s_%d(%d-%d,im=%g),nOOB=%d,nTree=%d,boot=%d,dep1=%d,eta=%g,lenda=%g,weak=%d minSet=%g...\r\n", sTitle.c_str(), cas,
           Trains.size() - nOOB, nOOB, sqrt(im0 / nSample), nOOB, nTree, boot, maxDepth + 1, eta, lenda, nPickAtSplit(nullptr), WeakLearner::minSet);
