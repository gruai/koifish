...
void RF_ShapeRegress::RandCandidate(int nCand, ShapeBMPfold &mean, ShapeBMPfold::PTFs &cands, int flag) {
    if (index == ShapeBMPfold::INTER_2) {
        RandCandidate_2(nCand, mean, cands, flag);
        return;
    }
    cands.clear();
    cands.resize(nCand);
...
void RF_ShapeRegress::RandCandidate_2(int nCand, ShapeBMPfold &mean, ShapeBMPfold::PTFs &cands, int flag) {
    ...
    for (i = 0; i < nCand; i++) {
        ShapeBMPfold::PTonF &pt = cands[i];
        pt.indx                 = ShapeBMPfold::INTER_2;
        pt.iU = iU = uniti(*hRander);
        assert(pt.iU >= 0 && pt.iU < nMark);
        do {
            pt.iV = uniti(*hRander);
        } while (pt.iU == pt.iV);
...
#endif
    printf("\n********* %s INDEX: %s nBlitThread=%d *********", regular == MULTI_TREE ? "MULTI" : "SINGLE", ShapeBMPfold::index_info(index).c_str(),
           nBlitThread);
...
RF_ShapeRegress::~RF_ShapeRegress() {
    ...
    if (cand_dis != nullptr)
        delete[] cand_dis;
    if (arrPTF != nullptr) {
        for (int i = 0; i < nBlitThread; i++) arrPTF[i].clear();
        delete[] arrPTF;
    };
}

double RF_ShapeRegress::ErrorAt(arrPFNO &samps) {
    int no, nSamp = samps.size(), nMark = Trains[0]->vS.rows();
    assert(nSamp > 0);
    double err = 0.0, a;
    for each (F4NO *fn in samps) {
        no = fn->pos;
        assert(no >= 0 && no < hTrainData->ldF);
...
hBLIT RF_ShapeRegress::GetBlit(WeakLearner *hWeak, int flag) {
    ...
    for (i = 0; i < nRand; i++) {  // 预先生成，避免多线程的不确定性
        do {
            iU = UniformInt(0, nPt - 1);
            iV = UniformInt(0, nPt - 1);
        ...
        no        = UniformInt(0, nSamp - 1);
        no        = hWeak->samps[no]->pos;
...
void RF_ShapeRegress::OnMultiTree(int cas, int nMulti, int flag) {
    ...
    for each (ShapeBMPfold *hFold in Trains) {
        ...
        if (no < nBoot) {  //
            ...
            a = hFold->NormalErr() / nOOB;
            eOOB += a;
            assert(hFold->nBag == nMulti);
        }
        hFold->nBag = 0;
        no++;
        hFold->vMomen.setZero();
        hFold->confi = 0.0;
    }
    err /= nBoot;
...
int RF_ShapeRegress::Train(string sTitle, int cas, int flag) {
    ...
    printf("\n********* %s_%d(%d-%d,im=%g),nOOB=%d,Step=%d,nCand=%d,dep1=%d,eta=%g,lenda=%g,weak=%d minSet=%g...\r\n", sTitle.c_str(), cas,
           Trains.size() - nOOB, nOOB, sqrt(im0 / nSample), nOOB, nTree, cands.size(), maxDepth + 1, eta, lenda, nPickAtSplit(nullptr), WeakLearner::minSet);
    ...
    for (skdu.step = 0; skdu.step < skdu.nStep; skdu.step++) {
        ...
        for (t = 0; t < EACH; t++, noT++) {
            ...
            DecisionTree *hTree = new DecisionTree(this, hTrainData);
            hTree->name         = to_string(666) + "_" + to_string(noT);
            ...
            if (regular == SINGLE_TREE && isCalcErr) {
                printf("%d: err=(%5.3g,%5.3g) eOOB=%g nTree=%d nWeak=%g,eta=%g\n", skdu.step, im1, hRoot->err, eOOB, forest.size(), nzWeak / (skdu.step + 1),
                       eta);
            }
            k = 0;
            for each (DecisionTree *hTree in forest) {
                if (isToCPP)
                    ToCPP(hTree, cas, skdu.step, k++, 0x0);
                delete hTree;
            }
            forest.clear();
        }
...
void RF_ShapeRegress::ToCPP(DecisionTree *hTree, int cas, int step, int tree, int flag) {
    ...
    for each (WeakLearner *hWeak in vNodes) {
        ...
        fwrite(bmove, sizeof(char), nRow * 2, fpD);
        nLeaf++;
#else
        fwrite(move, sizeof(float), nRow * 2, fpD);
        nLeaf++;
#endif
    }
    if (1) {
        ...
        for each (WeakLearner *hWeak in vNodes) {
            ...
            if (hBlit == nullptr) {
                nodes[0] = -1, nodes[1] = nLeaf * nRow * 2, nodes[2] = 0, nodes[3] = -1, nodes[4] = -1;
                nLeaf++;
            } else {
                nodes[0] = hBlit->id, nodes[1] = hBlit->other, nodes[2] = (int)(hBlit->thrsh), nodes[3] = hRegress->left->id, nodes[4] = hRegress->right->id;
            }
        ...
        for each (WeakLearner *hWeak in vNodes) {
            ...
            BLIT_Diff *hBlit = dynamic_cast<BLIT_Diff *>(hRegress->hBlit);
            if (hBlit == nullptr) {
                fprintf(fpC, "{%d,%d,%d,%d,%d},", -1, nLeaf * nRow * 2, 0, -1, -1);
                nLeaf++;
            } else {
                fprintf(fpC, "{%d,%d,%d,%d,%d},", hBlit->id, hBlit->other, (int)(hBlit->thrsh), hRegress->left->id,
                        hRegress->right->id);  //{	hBlit->id = iU;		hBlit->other = iV;	hBlit->thrsh=thrsh;		hBlit->gain=g;		}
            }
        }
        fprintf(fpC, "\n};\n");
...
void RF_ShapeRegress::CoreInCPP(int nCas, int flag) {
    ...
    for (i = 0; i < nCas; i++) {
        fprintf(fpC,
                "\t_CAS_%d_( gray,hei,ldW,shape,nodes,moves,roi,flag ); "
                "\tif(++i>=nMost)	return;\n",
                i);
    }
    fprintf(fpC, "}\n");
