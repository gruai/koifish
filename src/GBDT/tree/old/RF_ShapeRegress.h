...
class RF_ShapeRegress : public RandomForest {
   ...
   protected:
    ...
    virtual int nPickAtSplit(WeakLearner *hWeak) { return nPickWeak; }
   ...
   public:
    typedef enum { SINGLE_TREE, MULTI_TREE } REGULAR;
    REGULAR regular;

    typedef enum { BT_ALL, BT_MAX_ERR, BT_MIN_ERR, BT_RANDOM_3 } BOOT;
    BOOT boot;
