...
// δ��Ч��������֡��ƺ�����Ҫ�ſ��߽磬���������У���Щ���λ����roi֮��		1/6/2015
    ...
    try {
        ...
        do {
            if (BIT_TEST(data.dwFileAttributes, FILE_ATTRIBUTE_DIRECTORY)) {
                ...
                if (len <= 2 && (_tcscmp(data.cFileName, _T(".")) == 0 || _tcscmp(data.cFileName, _T("..")) == 0))
                    continue;
...
bool FaceDetect_(GST_BMP *hSrc, BIT_8 *gray, int wth, int hei, RoCLS &roi, int flag = 0x0) {
    ...
    double a;
    GST_TIC(tick);
    if (0) {                                                                       // ����̫�� 1 ��֧��64-bit 2 ׼ȷ�ʲ�
        if (FaceLocate_ge(gray, wth, hei, &left, &rigt, &botom, &top, 0x0) > 0) {  // ȡ��ƫ��
        } else
            return false;
    } else {  // debug��ʱ��ʵ��̫����Ī������
    ...
    if ((a = GST_TOC(tick)) > 1.0)
        printf("time=%g,", a);
    roi = RoCLS(left, botom, rigt - left, top - botom, 0, 0);
    if (0 && hSrc != nullptr) {
        BMP_DRAW *hTrace = new BMP_DRAW(hSrc);  // BMP_DRAW���ڴ�й©
        hTrace->Line(left, rigt, botom, botom);
        hTrace->Line(rigt, rigt, top, botom);
        hTrace->Line(left, rigt, top, top);
        hTrace->Line(left, left, top, botom);
        // printf( "%d",rigt-left );
        char sPath[1000];
...
bool NormalizeFace(ShapeBMPfold *hFold, RoCLS &roi, double *pts, int flag = 0x0) {
    ...
    bool isDump = true;
    r0          = roi.r_0 + roi.hei / 2;
    c0          = roi.c_0 + roi.wth / 2;
    for (i = 0; i < _FACE_MARK_0_; i++) {
        pts[2 * i]     = (pts[2 * i] - roi.c_0) * sZoom;
        pts[2 * i + 1] = (pts[2 * i + 1] - roi.r_0) * sZoom;
...
ShapeBMPfold *Fold_300wMarks(const string &sPath, int flag = 0x0) {
    hBMPP hbmp = new BMPP(sPath, szInital);
    hbmp->Median();
    ...
    if (fp != NULL) {
        while (fgets(sLine, 1000, fp)) {
            if (sscanf(sLine, "%f %f", &x, &y) == 2) {
                x       = x * hbmp->init.sW;
                y       = hei - y * hbmp->init.sH;  //"pts"�ļ�������ԭ�������Ͻ�
                c0      = MIN(c0, x);
                c1      = MAX(c1, x);
                r0      = MIN(r0, y);
                r1      = MAX(r1, y);
...
bool ReadLandMarks(string sPicPath, ShapeBMPfold *sfold, bool isDetect, string sOutDir, int flag = 0x0) {
    ...
    sMark += "_n";
    sfold->Serial(sMark, false); /**/
    if (0) {                     // ����У���ǵ������Ƿ�׼ȷ		2968784797_1.jpg�Ͳ�׼
        SHAPE_IMAGE si(_FACE_NW_, _FACE_NH_, 3);
        si.bpp    = 24;
        sfold->vS = sfold->vY;
...
void GetMeanShape(vector<ShapeBMPfold *> &Trains, ShapeBMPfold &meanShape, vector<ShapeBMPfold *> &means, int flag = 0x0) {
    ...
    /*double delta[9][2]={//ʧ�ܵĳ���			1/4/2016
...
bool LoadSamplesAt(wchar_t *sFolder, vector<ShapeBMPfold *> &Samples, int nMost = 100, int flag = 0x0) {
    ...
    // wMax=240,		hMax=320;		//̫С��
    char sPath[_MAX_PATH];
    for each (wstring wPath in g_arrPath) {
        if (arrPath.size() >= nMost)
            break;
...
#pragma omp parallel for num_threads(nThread) private(i)
    for (i = 0; i < nImage; i++) {
        ...
        ShapeBMPfold *spY = new ShapeBMPfold(hbm, spsv);
        spY->nam          = TITLE(arrPath[i]);
        GST_VERIFY(spY != nullptr, "spY is 0");
        if (ReadLandMarks(arrPath[i].c_str(), spY, isDetecFace, sObjTrainDir) || isTestLigner) {  // ����ͼƬ������.pts_n
#pragma omp critical
            {
    ...
    if (nSample > 0) {
        ...
        for (i = 0; i < nSample; i++) {
            unit(i) = Samples[i]->lenUnit;
            assert(unit(i) > 0 || Samples[i]->vY.size() == 0);
...
void TestLigner(vector<ShapeBMPfold *> &Samples, ShapeBMPfold::VECT &sp0, int flag = 0x0) {
    ...
    for each (ShapeBMPfold *hSamp in Samples) {
        hSamp->hbmp->no = no++;
        hSamp->vS       = sp0;
        hSamp->type     = ShapeBMPfold::SAMP_TEST;
        if (1) {
...
void GruFacer_Test() {
    ...
    // ����ͼƬ����
    for (i = 0; i < nImage; i++) {
        // GST_BMP bmp( _T("I:\\��������\\Face\\InsidePeople\\chuyanghao_police\\0.jpg") );
...
#ifdef _DEBUG
    for (i = 0; i < GFACE_FEAT_LEN; i++) printf("%g ", feat[i]);
#endif
    delete[] feat;
    delete[] info;
    GFACER_clear();
...
int _tmain(int argc, _TCHAR *argv[]) {
    ...
    if (!isTestLigner)
        LoadSamplesAt(sTrainSetDir, Samples, nMaxTrain);
    /*
        1 ������dup����Ч��
        2 nStep�ĺ���ֵ (20����100��10��why? )
    */
...
NEXT:
    ...
    if (isDetecFace || isTestLigner) {
        ...
        for each (ShapeBMPfold *hSamp in Trains) delete hSamp;
        Trains.clear();
        return 1;
    }
