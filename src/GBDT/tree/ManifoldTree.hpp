...
typedef enum { RF_UNDEF = 0, RF_TRAIN = 1, RF_TEST, RF_VERIFY } RF_STAGE;
...
//	typedef shared_ptr<WeakLearner> hWLEARN;
typedef shared_ptr<WeakLearner> hWLEARN;
typedef std::vector<shared_ptr<WeakLearner>> wLEARNERs;
...
class DecisionTree {
   ...
   public:
    ...
    bool isTrained(int flag = 0x0) { return hRoot()->feat >= 0; }
...
class ManifoldTree : public DecisionTree {
   ...
   public:
    ...
    std::priority_queue<hMTNode, std::vector<hMTNode>, _leaf_compare_> leafs;
