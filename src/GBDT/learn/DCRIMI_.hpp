...
namespace Grusoft{
	struct DCRIMI_ {
		...
		double f_ar_8[8], f_rr_8[8], hd_8[8];	//��Ӧ1.0e-7~1.0��8���̶�
		double CRR = 0, time = 0;
	...
	struct DCRIMI_2 {
		...
		double AUC_Jonson(size_t dim, const Tx *y0, const Tx *y1, int flag = 0x0) {
			...
			if (N_pos == 0) {
				...
				for (i = 0; i < dim; i++) {		//���ȶ�score�Ӵ�С����Ȼ�������score��Ӧ��sample ��rankΪn
					dot += y0[idx[i]]*(i+1);
				}
		...
		/*
		template<typename Tx>
		double AUC_cys_1(size_t dim, const Tx *label, const Tx *y1, int flag = 0x0) {	//ʧ�ܵĳ��ԣ���
			clock_t t1 = clock();
			...
			//ParallelSort(idx.begin(), idx.end(), [&y1](tpSAMP_ID i1, tpSAMP_ID i2) {return y1[i1] > y1[i2]; }, IteratorValType(idx.begin()));
			//tX += ((clock() - (t1))*1.0f / CLOCKS_PER_SEC);
			for (i = 0; i < nz; i++) {		//���ȶ�score�Ӵ�С����Ȼ�������score��Ӧ��sample ��rankΪn
				if (label[idx[i]] == 0)
					continue;
				dot += (dim-nz_1)-i;
			}
		...
		template<typename Tx>
		double AUC_cys(size_t dim, const Tx *label, const Tx *y1, int flag = 0x0) {	//ʧ�ܵĳ��ԣ���
			clock_t t1 = clock();
