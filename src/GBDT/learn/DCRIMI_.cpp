...
DCRIMI_::DCRIMI_(void *hB, int span, int flag) : D_span(span), hBase(hB), dump(1), isSaveFalse(false), isBidui(false) {
    ...
    D_inter = new float[D_span + 1];
    D_intra = new float[D_span + 1];
...
DCRIMI_::DCRIMI_(const DCRIMI_ &dcri) : isSaveFalse(dcri.isSaveFalse) {
    ...
    rFAR = dcri.rFAR, rFRR = dcri.rFRR, rEER = dcri.rEER, D_ = dcri.D_, sep = dcri.sep, eer_sep = dcri.eer_sep;
...
void DCRIMI_::Analyze(const string &sTitle, int flag) {
    ...
    for (i = 0; i <= D_span; i++) {
        ...
        nz_a += D_intra[i];
        if (D_intra[i] > 0) {
            max_a = MAX2(max_a, i * D_s);
            min_a = MIN2(min_a, i * D_s);
        }
        mean_r += i * D_inter[i];
        nz_r += D_inter[i];
        if (D_inter[i] > 0) {
            max_r = MAX2(max_r, i * D_s);
            min_r = MIN2(min_r, i * D_s);
        }
    }
    //	nz_a = nz_a;				nz_r = nz_r;
    ...
    if (dump != 0) {
        printf(
            ...
            sTitle.c_str(), nz_a, nz_r, mean_a, devia_a, max_a, min_a, mean_r, devia_r, max_r, min_r, accu, sep, rFRR, rFAR * 100, rEER, eer_sep);
        for (i = 0; i < 8; i++) {
