...
EnsemblePruning::EnsemblePruning(BoostingForest *hBoost_, FeatsOnFold *hFold_, int mWeak_, int flag) : hBoost(hBoost_), hFold(hFold_), nMostWeak(mWeak_) {
    ...
    for (size_t i = 0; i < nSamp; i++) wasSmall[i] = 1;
    ...
    if (isDebug) {
        nWeak = 98;
        LoadCSV("e:/EnsemblePruning_625_98_.csv", 0x0);
        Pick(0, 0, 1);
    }
}

/*
    ���Լ���"e:/EnsemblePruning_1250_18_.csv"��	score_1=0.8564785350613284��score_2=0.8487257000694284
*/
...
void EnsemblePruning::OnStep(ManifoldTree *hTree, tpDOWN *hWeak, int flag) {
    if (init_score == nullptr) {  // ����Ľ���	https://towardsdatascience.com/demystifying-maths-of-gradient-boosting-bd5715e82b7c
...
void EnsemblePruning::ToCSV(const string &sPath, int flag) {
    ...
    if (y != nullptr) {
        fprintf(fp, "%lf\t", nWeak * 1.0);
        fprintf(fp, "%lf\t", -6666.0);
    }
...
double EnsemblePruning::UpateGamma(int *isLive, int nY, int flag) {
    ...
    if (!isRand) {           // �����ڵ���
        double gamma_0[] = {-1.95880275, 0.03199851,  2.31866016, 1.13076601,  -0.47713595, 1.10638398, -0.16743563, 0.016846,    -0.63711211,
                            -0.77417962, -0.46160502, 0.24628335, -0.00904039, -0.39995661, 0.69108282, -1.26731069, -2.25347049, 1.29984439};
...
int EnsemblePruning::SubOnLive(double delta, bool update_orth, double *v_0, double *v_sub, int flag) {
    ...
    if (update_orth) {
        FREE_a(orth);
        ldOrth   = nY;
        num_orth = 0, orth = new double[ldOrth * ldOrth]();
    }
    return nY;
}
...
void EnsemblePruning::round_coloring(bool balanced, int flag) {
    double *samps = new double[nWeak * 6], *flips = samps + nWeak, *init_y = flips + nWeak, *sub_y = init_y + nWeak, *best_sub_y = sub_y + nWeak,
           *new_sub_y = best_sub_y + nWeak;
    RAND_normal(nWeak, samps);
    if (!isRand) {  // �����ڵ���
        // double samples_0[] = { 0.04892675, 0.85041767, 0.0882261, 0.00201122, 0.64102732, 0.26890527, 0.1720314, 0.76263232, 0.54824072, 0.14100026,
        // 0.17752911, 0.09100698 	,0.16327615, 0.34498547, 0.49066404, 0.44659881, 0.04286212, 0.94289195 };
...
bool EnsemblePruning::Pick(int nTree, int isToCSV, int flag) {
    ...
    for (cc_0_sum = 0, i = 0; i < nWeak; i++) {
        cc_0_sum += fabs(cc_0[i]);
    }
    ...
    for (i = 0; i < nWeak; i++) {
        cc_0[i] /= cc_0_sum;
        scale_(nSamp, mA + i, nWeak, cc_0[i]);  // scaled_a = np.multiply(sub_a,sub_x)	discrepancy_minimize������
    }
    // Compare(0x0);
    ...
    delete[] grad;
    for (i = 0; i < nWeak; i++) {
        scale_(nSamp, mA + i, nWeak, 1.0 / cc_0[i]);  // Compare������
        cc_0[i] *= cc_0_sum;
        cc_1[i] *= cc_0_sum;
    }
