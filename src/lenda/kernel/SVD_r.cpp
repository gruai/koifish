...
void submatrix_submatrix_mult_with_ab(mat *A, mat *B, mat *C, 
    ...
    cblas_dgemm(CblasRowMajor, (CBLAS_TRANSPOSE)transa, (CBLAS_TRANSPOSE)transb, 
...
int PerSVD(char *filename, int m, int n, int k, int l, int p, float *A,float *mU,int ldU, float *sigma,float *mVt,int ldV,int flag=0x0)       {
    ...
    matrix_copy(Q, QQ);
    matrix_delete(QQ);
    matrix_delete(D1);      matrix_delete(D2);      matrix_delete(st);
    eigSVD(Qt, Qt, SS, VV);
    ...
    matrix_delete(SiV);
    eigSVD(B, Q, SS, VV);
    matrix_delete(B);       matrix_delete(Sl);
    int inds[k];
    int s = l - k;
    ...
    matrix_delete(U);       matrix_delete(V);       
    return 0x0;
}
