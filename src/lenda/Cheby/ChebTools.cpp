...
template <class T>
bool inbetween(T bound1, T bound2, T val) {
    return val > std::min(bound1, bound2) && val < std::max(bound1, bound2);
}
...
Eigen::ArrayXd get_monomial_from_Cheb_basis(int n) { return monomial_coefficients_library.get_coeffs(n); }
...
const Eigen::VectorXd &get_CLnodes(std::size_t N) { return CLnodes_library.get(N); }
...
ChebyshevExpansion ChebyshevExpansion::operator+(const ChebyshevExpansion &ce2) const {
    if (m_c.size() == ce2.coef().size()) {
        ...
        if (m_c.size() > ce2.coef().size()) {
            Eigen::VectorXd c(m_c.size());
            c.setZero();
            c.head(ce2.coef().size()) = ce2.coef();
...
ChebyshevExpansion ChebyshevExpansion::operator*(double value) const { return ChebyshevExpansion(m_c * value, m_xmin, m_xmax); }
...
ChebyshevExpansion ChebyshevExpansion::operator*(const ChebyshevExpansion &ce2) const {
    std::size_t order1 = this->m_c.size() - 1, order2 = ce2.coef().size() - 1;
    ...
    Eigen::VectorXd a = Eigen::VectorXd::Zero(Norder_product + 1), b = Eigen::VectorXd::Zero(Norder_product + 1);
    a.head(order1 + 1) = this->m_c;
    b.head(order2 + 1) = ce2.coef();
...
const vectype &ChebyshevExpansion::coef() const { return m_c; };
/**
...
vectype ChebyshevExpansion::y_recurrence_xscaled(const vectype &xscaled) const {
    ...
    if (Norder == 0) {
        return m_c[0] * Eigen::MatrixXd::Ones(A.rows(), A.cols());
    }
    if (Norder == 1) {
        return m_c[0] + m_c[1] * xscaled.array();
    }
...
Eigen::MatrixXd ChebyshevExpansion::companion_matrix_noreduce(const Eigen::ArrayXd &coeffs) const { return rootfinding::companion_matrix(coeffs); }
...
std::vector<double> ChebyshevExpansion::real_roots2(bool only_in_domain) const {
    ...
    for (auto i = 0; i + 2 < Ndegree_scaled + 1; i += 2) {
        const double &x_1 = xscaled[i + 0], &y_1 = yy[i + 0], &x_2 = xscaled[i + 1], &y_2 = yy[i + 1], &x_3 = xscaled[i + 2], &y_3 = yy[i + 2];
...
double ChebyshevExpansion::monotonic_solvex(double y) const {
    ...
    */
    ...
    bool increasing = ynodes[ynodes.size() - 1] > ynodes[0];  // Nodes go from 1 to -1 (stuck with this), but increasing says whether the value at the last
                                                              // *index* (x=-1) is greater than that of the first index (x=1).
    if (increasing) {
...
Eigen::VectorXd ChebyshevExpansion::get_nodes_realworld() { return ((m_xmax - m_xmin) * get_nodes_n11().array() + (m_xmax + m_xmin)) * 0.5; }
...
ChebyshevExpansion ChebyshevExpansion::integrate(std::size_t Nintegral) const {
    ...
    // and example in https ://github.com/numpy/numpy/blob/master/numpy/polynomial/chebyshev.py#L868-L964
    if (Nintegral != 1) {
        throw std::invalid_argument("Only support one integral for now");
    }
