...
namespace ChebTools { namespace rootfinding {
...
/**
 ...
 \note The loss in precision of the roots can be sometimes quite significant, so much so that the obtained roots are useless, so use with caution, and primarily
 for testing
 */
inline auto explicit_Chebyshev_4x4(const Eigen::Matrix<double, 4, 4> &A4)
    ...
    double a = -3 * B * B / (8 * A * A) + C / A, b = B * B * B / (8 * A * A * A) - B * C / (2 * A * A) + D / A,
           c = -3 * B * B * B * B / (256 * A * A * A * A) + C * B * B / (16 * A * A * A) - B * D / (4 * A * A) + E / A;
    if (b == 0) {
        auto solns = std::make_tuple(-B / (4 * A) + sqrt1((-a + sqrt1(a * a - 4 * c)) / 2.0), -B / (4 * A) + sqrt1((-a - sqrt1(a * a - 4 * c)) / 2.0),
                                     -B / (4 * A) - sqrt1((-a + sqrt1(a * a - 4 * c)) / 2.0), -B / (4 * A) - sqrt1((-a - sqrt1(a * a - 4 * c)) / 2.0));
        return solns;
    } else {
        double P = -a * a / 12 - c;
        ...
        auto solns = std::make_tuple(
            -B / (4 * A) + (+W + sqrt1(-(3 * a + 2.0 * y + 2 * b / W))) / 2.0, -B / (4 * A) + (+W - sqrt1(-(3 * a + 2.0 * y + 2 * b / W))) / 2.0,
            -B / (4 * A) + (-W + sqrt1(-(3 * a + 2.0 * y - 2 * b / W))) / 2.0, -B / (4 * A) + (-W - sqrt1(-(3 * a + 2.0 * y - 2 * b / W))) / 2.0);
...
inline auto explicit_Chebyshev_3x3(const Eigen::Matrix<double, 3, 3> &A3) {
    ...
    auto sqrt3                 = sqrt(3.0);
    auto F                     = cbrt1(-27 * a31 - 4 * pow(a33, 3) - 9 * a33 * (a32 + 1) + 27 * a33 +
                                       sqrt1(-2 * pow(3 * a32 + 2 * pow(a33, 2) + 3, 3) + pow(-27 * a31 - 4.0 * pow(a33, 3) - 9 * a33 * (a32 + 1) + 27 * a33, 2)));
    std::complex<double> soln1 = (1.0 / 6.0) * (F * (-cbrt(2) * F + 2 * a33) - pow(2, 2.0 / 3.0) * (3 * a32 + 2 * pow(a33, 2) + 3)) / F;
    std::complex<double> soln2 =
        (1.0 / 12.0) * (F * (1.0 + sqrt3 * I) * (cbrt(2) * F * (1.0 + sqrt3 * I) + 4 * a33) + 4 * pow(2, 2.0 / 3.0) * (3 * a32 + 2 * pow(a33, 2) + 3)) /
        (F * (1.0 + sqrt3 * I));
    std::complex<double> soln3 =
        (1.0 / 12.0) * (F * (1.0 - sqrt3 * I) * (cbrt(2) * F * (1.0 - sqrt3 * I) + 4 * a33) + 4 * pow(2, 2.0 / 3.0) * (3 * a32 + 2 * pow(a33, 2) + 3)) /
        (F * (1.0 - sqrt3 * I));
...
}}  // namespace ChebTools::rootfinding
