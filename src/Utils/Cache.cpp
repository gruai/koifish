...
hGensor KVCache::SerialV(void *ctx, hGensor Vcur, int il, bool isSave) {
    ...
    int n_ctx = config.n_ctx(),n_batch = config.n_batch(),n_embd_v_gqa = config.n_embd_v_gqa(il),n_embd_head_v =
    config.n_embd_head_v,n_head_kv=config.n_head_kv(il); int kv_head=0,kv_size = kv->size;
...
hGensor KVCache::SerialK(void *ctx, hGensor Kcur, int il, bool isSave) {
    ...
    int n_ctx = config.n_ctx(),n_batch = config.n_batch(),n_embd_k_gqa  = config.n_embd_k_gqa(il),n_embd_head_k =
    config.n_embd_head_k,n_head_kv=config.n_head_kv(il); int kv_head=0,kv_size = kv->size;
