...
void GST_bitmap::Emboss() {
    ...
    if (IsValid()) {
        ...
        fixed f_w[9] = {itofx(-1), itofx(-1), itofx(0), itofx(-1), itofx(0), itofx(1), itofx(0), itofx(1), itofx(1)};
        fixed f_w1   = ftofx(0.299f);
        fixed f_w2   = ftofx(0.587f);
        fixed f_w3   = ftofx(0.114f);
...
void GST_bitmap::GlowingEdges(long blur, long threshold, long scale) {
    ...
    if (IsValid()) {
        ...
        for (long k = 0; k < blur; k++) tempBitmap.GaussianBlur();
            ...
            case RM_NEARESTNEIGHBOUR: {
                for (long i = _dstStartY; i <= _dstEndY; i++) {
                    for (long j = _dstStartX; j <= _dstEndX; j++) {
                        ...
                        float b   = (f_x - f_x0) * (f_y2 - f_y3 - f_y1 + f_y0) - (f_x1 - f_x0) * (f_y3 - f_y0) - (f_y - f_y0) * (f_x2 - f_x3 - f_x1 + f_x0) +
                                  (f_y1 - f_y0) * (f_x3 - f_x0);
...
void GST_bitmap::_DrawNearestNeighbour(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth,
                                       long srcHeight) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
...
void GST_bitmap::DrawTransparent(long dstX, long dstY, long width, long height, GST_bitmap &bitmapEx, long srcX, long srcY, long alpha,
                                 _PIXEL transparentColor) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
        long _dstStartX = max(0, min(m_bih.biWidth - 1, dstX));
...
void GST_bitmap::_DrawTransparentNearestNeighbour(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY,
                                                  long srcWidth, long srcHeight, _PIXEL transparentColor) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
        long _dstStartX = max(0, min(m_bih.biWidth - 1, dstX));
...
void GST_bitmap::_DrawTransparentBilinear(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth,
                                          long srcHeight, _PIXEL transparentColor) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
        long _dstStartX = max(0, min(m_bih.biWidth - 1, dstX));
...
void GST_bitmap::_DrawTransparentBicubic(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth,
                                         long srcHeight, _PIXEL transparentColor) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
...
void GST_bitmap::DrawTransparent(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth, long srcHeight,
                                 _PIXEL transparentColor) {
    // Check for resampling mode
    switch (m_ResampleMode) {
        case RM_NEARESTNEIGHBOUR: {
...
void GST_bitmap::_DrawTransparentNearestNeighbour(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY,
                                                  long srcWidth, long srcHeight, long alpha, _PIXEL transparentColor) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
        long _dstStartX = max(0, min(m_bih.biWidth - 1, dstX));
...
void GST_bitmap::_DrawTransparentBilinear(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth,
                                          long srcHeight, long alpha, _PIXEL transparentColor) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
        long _dstStartX = max(0, min(m_bih.biWidth - 1, dstX));
...
void GST_bitmap::DrawTransparent(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth, long srcHeight,
                                 long alpha, _PIXEL transparentColor) {
    // Check for resampling mode
    switch (m_ResampleMode) {
        case RM_NEARESTNEIGHBOUR: {
            // Draw bitmap using nearest neighbour interpolation algorithm
...
void GST_bitmap::DrawTransparent(_QUAD dstQuad, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth, long srcHeight, long alpha,
                                 _PIXEL transparentColor) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
...
void GST_bitmap::DrawBlended(long dstX, long dstY, long width, long height, GST_bitmap &bitmapEx, long srcX, long srcY, long startAlpha, long endAlpha,
                             DWORD mode) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
...
void GST_bitmap::_DrawBlendedNearestNeighbour(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth,
                                              long srcHeight, long startAlpha, long endAlpha, DWORD mode) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
        long _dstStartX = max(0, min(m_bih.biWidth - 1, dstX));
...
void GST_bitmap::_DrawBlendedBilinear(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth,
                                      long srcHeight, long startAlpha, long endAlpha, DWORD mode) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
...
void GST_bitmap::_DrawBlendedBicubic(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth,
                                     long srcHeight, long startAlpha, long endAlpha, DWORD mode) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
...
void GST_bitmap::DrawBlended(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth, long srcHeight,
                             long startAlpha, long endAlpha, DWORD mode) {
    // Check for resampling mode
    switch (m_ResampleMode) {
        case RM_NEARESTNEIGHBOUR: {
            // Draw bitmap using nearest neighbour interpolation algorithm
...
void GST_bitmap::_DrawCombinedNearestNeighbour(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth,
                                               long srcHeight, DWORD mode) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
        long _dstStartX = max(0, min(m_bih.biWidth - 1, dstX));
...
void GST_bitmap::_DrawCombinedBilinear(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth,
                                       long srcHeight, DWORD mode) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
...
void GST_bitmap::_DrawCombinedBicubic(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth,
                                      long srcHeight, DWORD mode) {
    // Check for valid bitmap
    if (IsValid() && bitmapEx.IsValid()) {
        // Calculate destination params
...
void GST_bitmap::DrawCombined(long dstX, long dstY, long dstWidth, long dstHeight, GST_bitmap &bitmapEx, long srcX, long srcY, long srcWidth, long srcHeight,
                              DWORD mode) {
    // Check for resampling mode
    switch (m_ResampleMode) {
        case RM_NEARESTNEIGHBOUR: {
...
void GST_bitmap::DrawTextA(long dstX, long dstY, LPSTR lpszText, _PIXEL textColor, long textAlpha, LPTSTR lpszFontName, long fontSize, BOOL bBold,
                           BOOL bItalic) {
    // Check for valid bitmap
    if (IsValid() && (lpszText != NULL)) {
        ...
        if (bBold)
            iWeight = FW_BOLD;
        HFONT hFont    = ::CreateFont(iFontHeight, 0, 0, 0, iWeight, bItalic, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                                      DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, lpszFontName);
...
void GST_bitmap::DrawTextW(long dstX, long dstY, LPWSTR lpszText, _PIXEL textColor, long textAlpha, LPTSTR lpszFontName, long fontSize, BOOL bBold,
                           BOOL bItalic) {
    // Check for valid bitmap
    if (IsValid() && (lpszText != NULL)) {
        // Draw text on the bitmap
            ...
            for (i = 0; i < m_bih.biHeight; i++) {
                for (j = 0; j < m_bih.biWidth; j++) {
                    if (bLarge) {
                        ...
                        m_lpFire[dwOffset] =
                            (BYTE)(((m_lpFire[dwOffset1] + m_lpFire[dwOffset2] + m_lpFire[dwOffset3] + m_lpFire[dwOffset4]) * _height) / _heightSize);
                        dwOffset++;
                    } else {
                        // Small fire
        ...
        for (long k = 0; k < iteration; k++) {
            ...
            for (long i = 1; i < m_bih.biHeight - 1; i++) {
                ...
                for (long j = 1; j < m_bih.biWidth - 1; j++) {
                    ...
                    long newHeight  = ((lpOldWaterMap[dwOffset1] + lpOldWaterMap[dwOffset2] + lpOldWaterMap[dwOffset3] + lpOldWaterMap[dwOffset4] +
                                       lpOldWaterMap[dwOffset5] + lpOldWaterMap[dwOffset6] + lpOldWaterMap[dwOffset7] + lpOldWaterMap[dwOffset8]) >>
                                      2) -
...
void GST_bitmap::GetRedChannelHistogram(long lpBuffer[256], BOOL bPercent) {
    ...
    if (IsValid() && (lpBuffer != NULL)) {
        ...
        if (bPercent) {
            ...
            for (long k = 0; k < 256; k++) lpBuffer[k] = (long)((float)lpBuffer[k] / (float)imageArea * 10000.0f);
        }
    }
}
