...
template <>
void LARNV<double>(int dim, double *X, int seed) {
...
template <>
bool isIMat<COMPLEXd>(int nCol, const hGMAT &mU) {
...
template <>
bool isTriangular<COMPLEXd>(int nCol, const hGMAT &mU, bool isUp) {
...
*/
template <>
void SchurDecomp<COMPLEXd>(int rank, hGMAT &mA, hGMAT &mU, hGVEC &vW, COMPLEXd info, int flag) {
...
void GECOV<float>(int dim, int n, float *X, double *dcov, double *dcor, int flag) {
    ...
    errcode = vslsSSEditTask(task, VSL_SS_ED_MIN, min_esti);
    CheckVslError(errcode);
    errcode = vslsSSEditTask(task, VSL_SS_ED_MAX, max_esti);
    CheckVslError(errcode);
    ...
    errcode = vslsSSEditCovCor(task, mean, (float *)cov, &cov_storage, (float *)cor, &cor_storage);
    CheckVslError(errcode);
