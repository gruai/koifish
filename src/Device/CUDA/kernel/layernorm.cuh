...
__global__ inline void fused_residual_forward_kernel6(floatX* residual, floatX* normed, float* mean, float* rstd,
    ...
    for(int c = threadIdx.x * x128::size; c < C; c += WARP_SIZE * x128::size) {
        ...
        for(int k = 0; k < x128::size; ++k) {
            float n = s * ((float)res[k] - m); // normalized output
            // float o = n * (float)w[k] + (float)b[k]; // scale and shift it
            float o = n * (float)w[k]; // scale and shift it
            out[k] = o;
        }
...
void inline layernorm_forward(floatX* out, float* mean, float* rstd,
    ...
    if(mean==nullptr){  //RMS
        ...
        rms_forward_kernel<<<grid_size, dim3(WARP_SIZE, block_y), smem, stream>>>(out, rstd, inp, weight, N, C);
    }else{
        auto status = cudaFuncSetAttribute(layernorm_forward_kernel6, cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
        cudaCheck(cudaGetLastError());
...
void inline fused_residual_forward5(floatX* residual, floatX* normed, float* mean, float* rstd,
    ...
    if(status == cudaSuccess) {
        if(bias==nullptr)
            fused_residual_forward_kernel6<<<grid_size, dim3(WARP_SIZE, block_y), smem, stream>>>(
                residual, normed,mean, rstd, inp1, inp2,weight, N, C);
        else{
            assert(mean!=nullptr);
...
void inline layernorm_backward(floatX* delta, floatX* dweight, floatX* dbias, float* scratch,
                        const floatX* deltaIn, const floatX* inp, const floatX* weight, const float* mean, const float* rstd,
                        int B, int T, int C, cudaStream_t stream) {
    NVTX_RANGE_FN();
    ...
    if(dbias==nullptr)
        ...
        assert(mean!=nullptr);
        layernorm_backward_kernel10<<<grid_size, block_size, shared_mem_size, stream>>>(delta, dweight, dbias, scratch, deltaIn, inp, weight, mean, rstd, B, T, C);
    }
