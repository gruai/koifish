...
hGTensor OutCLS::cuTrain(hGTensor inp_, int flag) {
    ...
    if (isForward()) {
        ...
        for (size_t i = 0; i < B; i += dB) {
            ...
            if (ToG0(w) != nullptr && delta != nullptr) {
                CU_mm_blas(ToX(delta) + nZ, ToX(w), ToX(cur) + off, NULL, C, dB * T, Vp, main_stream, false, false, 0, 0, 0, 0, false,
                           gelu_fusion >= 2 ? to_gelu : NULL, true);
                CU_mm_blas(ToG(w), z0 + nZ, ToX(cur) + off, NULL /*dbias*/, C, Vp, dB * T, main_stream, false, true, 0, 0, 0, 0, true /* accumulate */, NULL,
                           true);
            }
        }
