...
try{
    ...
    if (gelu_fusion < 1 && pre_gelu) {
        ...
        // swiglu_forward(rhs, pre_gelu, B*T*OC, main_stream);
    } else {
...
hGTensor FFN::FUSE_cuda(hGTensor hIn,int flag){
    ...
    if(isForward()){  
        ...
        up.Forw(tGelu,norm.out,remater_ffn?GTensor::tmpFF1:up.out);        //tGelu->Print("ffn.tGelu",0,dump_flag);
        // PrintTensor<floatX>("inp1",ToX(norm.out),true,B,T,C,1,-1);          PrintTensor<floatX>("ff1",ff1,true,B,T,latent,1,-1);  
        hGensor down_out = GTensor::tmpFF1;
        ...
        // GTensor::residual = out;
        out->Print("ffn.out",0,dump_flag);
    }else{
        assert(delta!=nullptr);        
        float*  scratchF = (float*) GTensor::buff;   // not the same inp1 of forward !!!
        if(remater_ffn){  
        ...
        layernorm_backward(ToX(GTensor::delta), ToG(norm.w), ToG0(norm.b), scratchF, ToX(delta), ToX(inp), ToX(norm.w), _mean, TO<float>(norm.rstd), B, T, C, main_stream);
        ...
        delta->Print("back of ffn0",0,dump_flag);
    }
