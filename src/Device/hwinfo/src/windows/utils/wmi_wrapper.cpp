...
namespace hwinfo { namespace utils { namespace WMI {
...
_WMI::_WMI() {
    ...
    if (locator) {
        ...
        if (service)
            res &= CoSetProxyBlanket(service, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, nullptr, RPC_C_AUTHN_LEVEL_CALL, RPC_C_IMP_LEVEL_IMPERSONATE, nullptr,
                                     EOAC_NONE);
    }
    if (!SUCCEEDED(res)) {
...
template <>
std::vector<unsigned long long> query(const std::wstring &wmi_class, const std::wstring &field, const std::wstring &filter) {
