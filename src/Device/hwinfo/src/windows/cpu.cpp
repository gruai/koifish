...
int64_t CPU::currentClockSpeed_MHz(int thread_id) const {
    auto data = utils::WMI::query<std::string>(L"Win32_PerfFormattedData_Counters_ProcessorInformation", L"PercentProcessorPerformance");
    if (data.empty()) {
        return -1;
    }
...
double CPU::currentUtilisation() const {
    auto res = utils::WMI::query<std::string>(L"Win32_PerfFormattedData_Counters_ProcessorInformation", L"PercentProcessorUtility",
                                              L"Name=" + std::to_wstring(0) + L",_Total");
    if (res.empty()) {
        return -1.f;
    }
...
double CPU::threadUtilisation(int thread_id) const {
    auto data = utils::WMI::query<std::string>(L"Win32_PerfFormattedData_Counters_ProcessorInformation", L"PercentProcessorUtility");
    if (data.empty()) {
        return -1.f;
    }
