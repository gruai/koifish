...
struct ggml_tensor * ggml_cross_entropy_loss_back_1(
        struct ggml_context         * ctx,
        struct ggml_tensor          * a,
        struct ggml_tensor          * b,
        struct ggml_tensor          * c) {
    GGML_ASSERT(ggml_is_scalar(c));
    struct ggml_tensor * result = ggml_dup_tensor(ctx, a);
    result->op   = GGML_OP_CROSS_ENTROPY_LOSS_BACK_1;
    result->grad = NULL;
    result->src[0] = a;
    result->src[1] = b;
    result->src[2] = c;
...
static void ggml_compute_forward_cross_entropy_loss_f32_1(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
#ifndef NDEBUG
        for (int i = 0; i < nc; ++i) {
            assert(!isnan(st[i]));            assert(!isinf(st[i]));
        }
#endif*/
    }
    ggml_barrier(params->threadpool);
...
//
static void ggml_compute_forward_cross_entropy_loss_back_f32_1(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_CROSS_ENTROPY_LOSS_1:
            {
                ggml_compute_forward_cross_entropy_loss_f32_1(params, tensor);
            }
            break;
        ...
        case GGML_OP_CROSS_ENTROPY_LOSS_BACK_1:
            {
                ggml_compute_forward_cross_entropy_loss_back_f32_1(params, tensor);
            }
            break;
        case GGML_OP_NONE:
            {
