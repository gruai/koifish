...
int _llama_build_graph(llama_model * model,struct ggml_cgraph **hgf,struct ggml_cgraph **hgb,int flag){
    ...
    struct llm_build_context llm(*lctx, ubatch, cb, false);
    llm.init();
    ggml_cgraph *gf = llm.build_llama( ),*gb = nullptr;
    if (lctx->cparams.embeddings) {
    ...
    ggml_backend_sched_alloc_graph(lctx->sched, gb);
    llama_set_inputs(*lctx, ubatch);
...
struct ggml_cgraph * _llama_raw_graph(llama_model * model,struct ggml_cgraph *gfx,const std::string&prompt,bool isOnline,int flag){
    ...
    if(0){
        tokens.resize(512);
        int32_t n_tokens = llama_tokenize(&(model->vocab),prompt.data(),prompt.size(),tokens.data(),tokens.size(), true, true);
        tokens.resize(n_tokens);         
    }
       
        
    llama_batch batch_all = llama_batch_get_one(tokens.data(), std::min(tokens.size(), (size_t)cparams.n_batch));
