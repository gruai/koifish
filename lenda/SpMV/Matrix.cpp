...
template<> hGVEC MatCCS<D>::Transform(hGVEC &vecY, const hGVEC &vecX, int flag )	 {
...
template<> hGMAT& MatCCS<D>::Transform(hGMAT &mV, const hGMAT &mU, int flag )	{		//V=AU
...
void TRI_1_Z( int dim,int** colptr,int** rowind,Z **val,Z**rhs )	{	
	...
	*colptr[0]=0;
	for( i = 0; i < dim; i++ )	{
		if( i>0 )	{
			(*rowind)[pos]=i-1;			(*val)[pos++]=s3;
		}
		(*rowind)[pos]=i;			(*val)[pos++]=s2;
		if( i<dim-1 )	{
			(*rowind)[pos]=i+1;			(*val)[pos++]=s1;
		}
		(*colptr)[i+1]=pos;
	}
	assert( pos==nz );
